...
// macros
#include "macros/RooMsgHandler.C"
#include "macros/makeAsimovData.C"
#include "macros/morphingProjection.C"
...
namespace LimitCrossCheck {
  ...
  bool    g_plotMorphingAnimations = true;  // Create animated gifs for morphing projections
  ...
  ostringstream       globalProblemMessages;
  TString             OutputDir;
  ...
  void          PlotMorphingControlPlots(double min = -2., double max = 2., size_t bins = 50);
  ...
  void                           SetPOIs(double mu);
  void                           SetPOIs(std::vector<double>& v_mu);
  void                           FixPOIs(double mu);
  void                           SetStyle();
  ...
  RooFitResult* FitPDF(ModelConfig* model, RooAbsPdf* fitpdf, RooAbsData* fitdata, TString minimType)
  {
    ...
    */
    ...
    while( (arg = (RooRealVar*)nItr->Next()) ) {
      ...
      if( TString(arg->GetName()).Contains("gamma_MC") ) {
        arg->setVal(1);
        continue;
      }
    ...
    // current default tolerance of 0.01
    // https://root.cern/doc/master/MinimizerOptions_8cxx_source.html#l00033
  ...
  void PlotHistosBeforeFit(double nSigmaToVary, double mu)
  {
    ...
    std::ostringstream badBins;
    RooMsgHandler      msgHandler(OutputDir, __func__, globalProblemMessages);
    const bool         addDirStatus = TDirectory::AddDirectoryStatus();
    ...
    while( (tt = (RooCatType*)iter->Next()) ) {
      ...
      while( (var = (RooRealVar*)it->Next()) ) {
        ...
        if( MapNuisanceParamNom[varname] != 0.0 && MapNuisanceParamNom[varname] != 1.0 )
          continue;
        ...
        TString histName;
        TString histNameStub = chanName + "_" + varName + "_" + TString(plotRelative) + "_";
...

        {
          ...
          SetPOI(mu);
          histName = histNameStub + "m1sigma";
          unique_ptr<TH1> hm1sigma(pdftmp->createHistogram(histName, *obs, Scaling(false)));
          hm1sigma->Scale(pdftmp->expectedEvents(*obs) / hm1sigma->Integral());

          // +1 sigma
          SetNuisanceParaToSigma(var, +nSigmaToVary);
          SetPOI(mu);
          histName = histNameStub + "p1sigma";
          unique_ptr<TH1> hp1sigma(pdftmp->createHistogram(histName, *obs, Scaling(false)));
          hp1sigma->Scale(pdftmp->expectedEvents(*obs) / hp1sigma->Integral());

          // Nominal
          SetNuisanceParaToSigma(var, 0.0);
          SetPOI(mu);
          histName = histNameStub + "nominal";
          unique_ptr<TH1> hnominal(pdftmp->createHistogram(histName, *obs, Scaling(false)));
          hnominal->Scale(pdftmp->expectedEvents(*obs) / hnominal->Integral());

          // Data
          histName = histNameStub + "data";
          unique_ptr<TH1> hdata(datatmp->createHistogram(histName, *obs));
          for( int ib = 0; ib < hdata->GetNbinsX() + 1; ib++ )
            hdata->SetBinError(ib, sqrt(hdata->GetBinContent(ib)));

          unique_ptr<TCanvas> c2(DrawShift(chanName, (TString)var->GetName(), expName, mu, hdata.get(), hnominal.get(),
                                           hp1sigma.get(), hm1sigma.get()));
          /*
           if(!c2) { continue; }
           */
        ...
        while( (comp = (RooProduct*)funcIter.Next()) ) {
          ...
          msgHandler.limitMemoryFootprint();
          ...
          unique_ptr<TH1> hm1sigma(comp->createHistogram(histName, *obs));
          ...
          unique_ptr<TH1> hp1sigma(comp->createHistogram(histName, *obs));
          ...
          unique_ptr<TH1> hnominal(comp->createHistogram(histName, *obs));
          ...
          */
          ...
          unique_ptr<TCanvas> c2(DrawShift(chanName, (TString)var->GetName(), compName, mu, 0, hnominal.get(),
                                           hp1sigma.get(), hm1sigma.get()));
        ...
        if( !IsAllStatDone ) {
          ...
          SetAllNuisanceParaToSigma(0.0);
          SetPOI(mu);
          histNameStub = chanName + "_Stat_" + TString(plotRelative) + "_";
          histName     = histNameStub + "m1sigma";
          unique_ptr<TH1> hm1sigma(pdftmp->createHistogram(histName, *obs));
          hm1sigma->Scale(pdftmp->expectedEvents(*obs) / hm1sigma->Integral());
          ...
          SetPOI(mu);
          histName = histNameStub + "p1sigma";
          unique_ptr<TH1> hp1sigma(pdftmp->createHistogram(histName, *obs));
          hp1sigma->Scale(pdftmp->expectedEvents(*obs) / hp1sigma->Integral());
          ...
          SetPOI(mu);
          histName = histNameStub + "nominal";
          unique_ptr<TH1> hnominal(pdftmp->createHistogram(histName, *obs));
          hnominal->Scale(pdftmp->expectedEvents(*obs) / hnominal->Integral());
          ...
          unique_ptr<TCanvas> c4(
              DrawShift(chanName, "Stat", expName, mu, hdata.get(), hnominal.get(), hp1sigma.get(), hm1sigma.get()));
          ...
          // Do a logarithmic signal vs background plot (independent of external value of mu)
          // and search for bins with suspiciously high signal/low background
          {
            // Bkg only
            SetAllStatErrorToSigma(0.0);
            SetNuisanceParaToSigma(var, 0.0);
            SetPOI(0.);
            histName = histNameStub + "bkg_only";
            unique_ptr<TH1> hbkgOnly(pdftmp->createHistogram(histName, *obs));
            hbkgOnly->Scale(pdftmp->expectedEvents(*obs) / hbkgOnly->Integral());

            // Bkg only -1 sigma stat
            SetAllStatErrorToSigma(-nSigmaToVary);
            histNameStub = chanName + "_Stat_" + TString(plotRelative) + "_";
            histName     = histNameStub + "bkgOnly_m1sigma";
            unique_ptr<TH1> bkgOnly_m1sigma(pdftmp->createHistogram(histName, *obs));
            bkgOnly_m1sigma->Scale(pdftmp->expectedEvents(*obs) / bkgOnly_m1sigma->Integral());

            // Bkg only +1 sigma stat
            SetAllStatErrorToSigma(+nSigmaToVary);
            histName = histNameStub + "bkgOnly_p1sigma";
            unique_ptr<TH1> bkgOnly_p1sigma(pdftmp->createHistogram(histName, *obs));
            bkgOnly_p1sigma->Scale(pdftmp->expectedEvents(*obs) / bkgOnly_p1sigma->Integral());

            // Sig
            SetAllStatErrorToSigma(0.0);
            SetPOI(1.);
            histName = histNameStub + "sig";
            unique_ptr<TH1> hsig(pdftmp->createHistogram(histName, *obs));
            hsig->Scale(pdftmp->expectedEvents(*obs) / hsig->Integral());
            hsig->Add(hbkgOnly.get(), -1.);

            unique_ptr<TCanvas> c5(DrawShift(chanName, "SignalVsBkg", "AllBkg(#mu=0)", 0, hdata.get(), hbkgOnly.get(),
                                             bkgOnly_p1sigma.get(), bkgOnly_m1sigma.get(), hsig.get()));
            c5->Write();
            if( drawPlots ) {
              dirName = OutputDir + "/" + MainDirSyst->GetName() + "/" + MaindirName + "/" + chanName + "/";
              c5->Print(dirName + "/SignalVsBkg.eps");
            }
            c5->Close();

            // Sanity check for predicted bin contents
            for( unsigned int bin = 0; bin <= (unsigned int)hbkgOnly->GetNbinsX() + 1; ++bin ) {
              double nSig = hsig->GetBinContent(bin);
              double nBkg = hbkgOnly->GetBinContent(bin);
              if( !std::isfinite(nSig) || !std::isfinite(nBkg) || (nSig != 0. && nSig >= 3. * nBkg) ) {
                badBins << setw(30) << left << chanName + ","
                        << "bin" << right << setw(4) << bin << defaultfloat << " [" << setw(6) << setprecision(3)
                        << hsig->GetXaxis()->GetBinLowEdge(bin) << ", " << setw(6) << setprecision(3)
                        << hsig->GetXaxis()->GetBinUpEdge(bin) << "]" << scientific << setprecision(3)
                        << ":\tS=" << setw(6) << nSig << "\tB=" << setw(6) << nBkg << std::endl;
              }
            }
          }
        }
    ...
    TDirectory::AddDirectory(addDirStatus);
    gROOT->SetBatch(false);

    TString problBinsFilename =
        OutputDir + "/" + MainDirSyst->GetName() + "/" + MaindirName + "/" + "problematicBins.txt";

    if( badBins.tellp() ) {
      globalProblemMessages << "\n\n"
                            << "=====================================================================\n"
                            << "=== Bins with non-finite bin contents or suspiciously high signal ===\n"
                            << "=== (Risking negative bin contents for mu <~ 0                  ) ===\n"
                            << "=====================================================================\n"
                            << badBins.str() << endl;
    }
                     ...
                     TH1* hsig)
  {
    ...
    if( hsig ) {
      max = std::max(hsig->GetMaximum(), max);
      vector<double> mins;
      for( auto hist : {m1s, p1s, n, d, hsig} ) {
        if( hist && hist->GetMinimum() != 0. )
          mins.push_back(hist->GetMinimum());
      }
      n->SetMinimum(mins.empty() ? 1.E-15 : *min_element(begin(mins), end(mins)) / 10.);
      pad1->SetLogy(1);
      n->SetMaximum(10. * max);
    } else {
    ...
    if( hsig ) {
      leg->AddEntry(hsig, "Signal (#mu=1)", "l");
    }
  ...
  void PlotMorphingControlPlots(double min, double max, size_t bins)
  {
    ...
    while( (tt = (RooCatType*)iter->Next()) ) {
      ...
      // Get pdf associated with state from simpdf
      unique_ptr<RooAbsPdf> pdftmp(simPdf->getPdf(tt->GetName()));
      RooArgSet*            obstmp = pdftmp->getObservables(*mc->GetObservables());
      RooRealVar*           obs    = ((RooRealVar*)obstmp->first());
      ...
      while( (var = (RooRealVar*)it->Next()) ) {
        ...
        unique_ptr<TCanvas> c2(new TCanvas(cname));
        c2->cd();
        unique_ptr<TH1> hh(pdftmp->createHistogram("hh_" + cname, *obs, YVar(*var, Binning(bins, min, max))));
        hh->SetLineColor(kBlue);
        hh->GetZaxis()->SetTitleOffset(2.5);
        hh->Draw("surf3 z");

        if( g_plotMorphingAnimations ) {
          project(hh.get(), OutputDir + "/Morphing/" + chanName + "/" + varName + "/" + "Total" + ".gif");
        }
        ...
        while( (comp = (RooProduct*)funcIter.Next()) ) {
          ...
          unique_ptr<TCanvas> c3(new TCanvas(cname));

          unique_ptr<TH1> hh(comp->createHistogram("hh_" + cname, *obs, YVar(*var, Binning(bins, min, max))));
          hh->SetLineColor(kBlue);
          hh->GetZaxis()->SetTitleOffset(2.5);
          hh->Draw("surf3 z");

          if( g_plotMorphingAnimations ) {
            project(hh.get(), OutputDir + "/Morphing/" + chanName + "/" + varName + "/" + compName + ".gif");
          }
  ...
  void PlotHistosAfterFitEachSubChannel(bool IsConditionnal, double mu)
  {
    ...
    while( (tt = (RooCatType*)iter->Next()) ) {
      ...
      RooMsgHandler msgHandler(OutputDir, TString(__func__) + tt->GetName(), globalProblemMessages);
      ...
      // Fit
      if( IsConditionnal && firstPOI ) {
        // for conditional fit, set all POIs to unity
        FixPOIs(mu);
      }
      ...
      TIterator*          itmh = dynamic_cast<TIterator*>(mc->GetParametersOfInterest()->createIterator());
      RooRealVar*         nmh  = (RooRealVar*)itmh->Next();
      while( nmh ) {
        muhat.push_back(nmh->getVal());
        nmh->setConstant(kFALSE);
        nmh = (RooRealVar*)itmh->Next();
      }
      ...
      while( (var = (RooRealVar*)it2->Next()) ) {
        ...
        h1Dpull->SetPointEXlow(ib - 1, h1Dpull_axis->GetBinWidth(ib) / 6.0);
        h1Dpull->SetPointEXhigh(ib - 1, h1Dpull_axis->GetBinWidth(ib) / 6.0);
        ...
        if( IsTooPulled || IsOverConst || IsOnesided ) {
          ...
          myGraph->SetPointEXlow(0, h1Dpull_axis->GetBinWidth(ib) / 6.0);
          myGraph->SetPointEXhigh(0, h1Dpull_axis->GetBinWidth(ib) / 6.0);
      ...
      _1sigma->Draw("F");
      h1Dpull->Draw("P");
      for( unsigned i = 0; i < vec_MyGraph.size(); i++ )
        vec_MyGraph[i]->Draw("P");
      ...
      for( int i = 0; i < frame->numItems(); i++ ) {
        ...
        if( obj_name == "" || obj_name.Contains("NotAppears") || obj_name.Contains("NoStacked") )
          continue;
    ...
    while( (tt1 = (RooCatType*)iter1->Next()) ) {
      Nchannel++;
      if( Nchannel == 1 )
        NPvecForName = MapChannelNPs[(TString)tt1->GetName()];
    }
    ...
    for( int isys = 0; isys < Nsyst; isys++ ) {
      ...
      while( (tt2 = (RooCatType*)iter2->Next()) ) {
        ...
        hsys->SetPointEXlow(ich - 1, hsys_axis->GetBinWidth(ich) / 6.0);
        hsys->SetPointEXhigh(ich - 1, hsys_axis->GetBinWidth(ich) / 6.0);
  ...
  void PlotHistosAfterFitGlobal(bool IsConditionnal, double mu, bool isAsimov)
  {
    ...
    if( !IsConditionnal && firstPOI && isAsimov )
      SetPOIs(1.1); // kick muhat in Asimov fits
    if( IsConditionnal && firstPOI && isAsimov ) {
    ...
    msgHandler.newCallerName(__func__ + string("_(NLLProfiles)"));
    ...
    TH1F*              h1Dpull_axis = new TH1F(h1name, h1name, NparNotStat, 0, NparNotStat);
    TGraphAsymmErrors* h1Dpull      = new TGraphAsymmErrors(NparNotStat);
    TGraph*            h1Dpull_real = new TGraph(NparNotStat);
    h1Dpull_axis->GetYaxis()->SetRangeUser(-5.5, 5.5);
    h1Dpull_axis->GetXaxis()->SetTitle("#theta");
    h1Dpull_axis->GetYaxis()->SetTitle("(#theta_{fit} - #theta_{0}) / #Delta#theta");
    ...
    while( (var = (RooRealVar*)it2->Next()) ) {
      ...
      TString     vname3 = var->GetName();
      RooRealVar* glob   = (RooRealVar*)(globObs->find("nom_" + vname3));
      // if nullptr, means it's a NP without constraint => floating norm or POI => pull = 0
      if( glob != nullptr ) {

        // ok, now look for MC stat NP, as they beave differently
        // WARNING: the following works for the "Poisson" parameterization of MC stat NP.
        // Code should be adapted for Gaussian param if needed.
        if( vname3.Contains("gamma_stat_") ) {
          globObs_val = 1;
          globObs_sig = 1 / sqrt(glob->getVal());
        }

        if( pull > globObs_val ) {
          if( fabs(errorLo) > globObs_sig ) {
            real_pull = 3; // default value. Large, but not too large that it's out of plot range
          } else {
            real_pull = (pull - globObs_val) / sqrt(globObs_sig * globObs_sig - errorLo * errorLo);
          }
        } else {
          if( errorHi > globObs_sig ) {
            real_pull = -3; // default value. Large, but not too large that it's out of plot range
          } else {
            real_pull = (pull - globObs_val) / sqrt(globObs_sig * globObs_sig - errorHi * errorHi);
          }
        }
      }
      // cout << "The REAL thing " << real_pull << '\n';
      h1Dpull_real->SetPoint(ib - 1, xc, real_pull);

      // Put in red NP with pull>1.5 OR Err<0.2, in black otherwise
      bool IsTooPulled = fabs(real_pull) > PullMaxAcceptable;
      bool IsOverConst = (fabs(errorLo) + fabs(errorHi)) / 2.0 < ErrorMinAcceptable;
      bool IsOnesided  = UseMinosError && (errorHi == 0 || errorLo == 0);
      if( IsTooPulled || IsOverConst || IsOnesided ) {
        TGraphAsymmErrors* myGraph = new TGraphAsymmErrors(1);
    ...
    h1Dpull->SetMarkerStyle(21);
    h1Dpull->SetMarkerSize(1.2);
    h1Dpull_real->SetMarkerStyle(25);
    h1Dpull_real->SetMarkerSize(1.2);
    _2sigma->SetFillColor(5);
    _2sigma->SetLineColor(5);
    _2sigma->SetMarkerColor(5);
    _1sigma->SetFillColor(3);
    ...
    h1Dpull_real->GetXaxis()->LabelsOption("v");
    _2sigma->Draw("F");
    _1sigma->Draw("F");
    h1Dpull->Draw("P");
    h1Dpull_real->Draw("P");
    ...
    if( draw1DResponse ) {
      ...
      gROOT->cd();
      RooAbsReal* nll = simPdf->createNLL(*localData, Offset(), Optimize(), NumCPU(2));
                         ...
                         TDirectory* tdir, TString snapshotName)
  {
    ...
    if( IsFloating )
      delta = 0.3;
    if( IsFloating && vname.Contains("ultijet") )
      delta = 1.0;
    if( vname.Contains("SigXsec") )
      delta = 1.0;
    // float delta = 3 * err;
  ...
  void PrintSuspiciousNPs()
  {
    ...
    if( !globalProblemMessages.str().empty() ) {
      cout << "\n\n\n"
           << "===========================================\n"
           << "=== Other problems\n"
           << "===========================================\n"
           << globalProblemMessages.str();
    }
    cout << endl;
    cout << endl;
    cout << "===================================================================" << endl;
  ...
  void GetNominalValueNuisancePara()
  {
    TIterator*  it  = mc->GetNuisanceParameters()->createIterator();
    RooRealVar* var = NULL;
    if( MapNuisanceParamNom.size() > 0 )
      MapNuisanceParamNom.clear();
  ...
  void FixPOIs(double mu)
  {
    TIterator*  it = dynamic_cast<TIterator*>(mc->GetParametersOfInterest()->createIterator());
  ...
  void FillGraphIntoHisto(TGraph* pGraph, TH1F* pHisto)
  {
    ...
    for( Int_t k = 0; k < pHisto->GetNbinsX(); k++ ) {
      ...
      // average:
      if( MatchingPoints > 0 )
        Sum = Sum / (Double_t)MatchingPoints;
  ...
  void PrintNumberOfEvents(RooAbsPdf* pdf)
  {
    ...
    while( (arg = (RooRealVar*)itr->Next()) ) {
      ...
      double x_hi  = 0;
      if( s_nom )
        x_hi = (s_hi - s_nom) / s_nom;
      double x_lo = 0;
      if( s_nom )
        x_lo = (s_lo - s_nom) / s_nom;
      double y_nom = b_nom;
      double y_hi  = 0;
      if( b_nom )
        y_hi = (b_hi - b_nom) / b_nom;
      double y_lo = 0;
      if( b_nom )
        y_lo = (b_lo - b_nom) / b_nom;
