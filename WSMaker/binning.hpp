...
class BinningTool {
 ...
 public:
  ...
  /**
   * @brief Smooth a syst histogram with a kernel method
   *
   * @param hnom Nominal histogram
   * @param hsys Systematic histogram
   * @param bins Vector of bin edges for the rebinning used to smooth
   * @param sm   can be smoothRatioUniformKernel or smoothDeltaUniformKernel
   */
  void smoothHistoKernel(TH1* hnom, TH1* hsys, const std::vector<int>& bins, SysConfig::Smooth sm);

  /**
   * @brief Function that actually performs the kernel smoothing
   *
   * @param h            Nominal histogram
   * @param h_up         Systematic histogram
   * @param span_up      Bandwidth to use
   * @param chi2_up      chi2 difference of nominal to smoothed
   * @param sm           can be smoothRatioUniformKernel or smoothDeltaUniformKernel
   * @param pointRemoved point in the histogram to be removed
   */
  void removePoint(TH1* const h, TH1* h_up, float span_up, double& chi2_up, SysConfig::Smooth sm,
                   const int pointRemoved = -1);

  /**
   * @brief Function that caclulates chi2 between nominal and smoothed histogram
   *
   * @param h_nom Nominal histogram
   * @param h_smooth Systematic histogram
   */
   ...
   * @param corr Correlation matrix to propagate stat correlation between bins in case of smoothing
   */
  ...
  // vector of floait bin boundaries (yes, I am sloppy and I am putitng as public memeber)
  std::vector<float> p_binningF;
};
