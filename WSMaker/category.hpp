...
class Category {
 ...
 private:
  ...
  // conf if the configuration for treatment and correlations
  /**
   ...
   * * scale the histo for the syst if requested in @c conf
   ...
   *
   * @conf{ApplyKernelSmoothing, false} the logic should not be here
   * @conf{KernelSmoothType, "SmoothRatio"} the logic should not be here
   * @conf{AverageSmoothedSys, false} the logic should not be here
   * @conf{ScaleSystematics, false} decide if the systematics scale to reduece
   * the difference between syst. and nominal
   *
   ...
   */
  void applyOneHistoSyst(const TString& name, const Systematic::Side side, const TString& dirname,
 ...
 public:
   ...
   *
   * Call the private @c applyOneHistoSyst with information on the given syst.
   *
  ...
  /**
   * @brief Get a histogram, from a single sample for a given sysname
   *
   *
   * The caller DOES NOT take ownership of the returned histogram
   *
   * @param s           The @Sample for which the histo is requested
   * @param sysname     The @TString name of the systematic to be called
   * NB. sysname = "" retrieves nominal. For sysname != "" please specify the *full*
   * name as stored in the inputs eg. with suffix __1up and __1down;
   * @return Pointer to the histogram; nullptr is no histogram exist for this category
   */
  TH1* getHist(const Sample& s, const TString& sysname = "") const;
  ...
  /**
   * @brief Accessor for variable size binning
   */
  const std::vector<float> getBinningFloat() const { return m_binningF; }

  /**
   * @brief Check for systs with same-sign effects on the sum of backgrounds
   *
   */
  void checkSameSignSysts() const;

  /**
   * @brief Creation of stat uncertainty histograms in a format required by the HistFactory
   *
   * This add up histograms of MC stat uncertainties in quadrature and then calculate the relative uncertainties to put
   * them in ShapeSys
   */
  void computeMCStatUncertHist();

  /**
   * @brief for some the systematics, we want to test the impact of them by varing the difference between the 1up/1down
   * and nominal
   *
   * This is done bin by bin, var = nom + (var - nom ) * factor, if the factor == 1, nothing changed, the factor
   * depending on the systematics
   *
   * @param type	The @c TString for which the systematics to be scaled
   * @return factor used for the scaling @c double
   */
  double getSystematicScalingFactor(TString sysname);
};
