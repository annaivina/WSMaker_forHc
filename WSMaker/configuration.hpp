...
class Configuration {
 ...
 public:
   ...
   */
  inline int getValue(const char* name, const int dflt) const { return m_env.get()->GetValue(name, dflt); }
  ...
  {
    return static_cast<bool>(m_env.get()->GetValue(name, static_cast<int>(dflt)));
  }
   ...
   * @return The value found in the @c TEnv
   */
  inline double getValue(const char* name, const double dflt) const { return m_env.get()->GetValue(name, dflt); }
   ...
   * @return The value found in the @c TEnv
   */
  inline TString getValue(const char* name, const char* dflt) const { return m_env.get()->GetValue(name, dflt); }
