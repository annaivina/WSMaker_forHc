...
class Engine {
 ...
 public:
   ...
   * @param ana A pointer to the @c Analysis to be run
   */
  Engine(const TString& confFileName, const TString& version, std::unique_ptr<Analysis>&& ana);
   ...
   * summary of pruning).
   * @conf{DoPruneSyst, true} apply systematics pruning.
   * @conf{PruneAfterMerge, false} apply systematics pruning after merging of samples
   * @conf{SmoothAfterMerge, false} do smoothing of systematics after merging of samples
   * @conf{SymmetriseAfterSmooth, false} do symmetrising of systematics after smoothing
   *
   */
  void MakeIt();
   ...
   *
   */
  TString PimpMyWS(const TString& wsfilename);
 ...
 private:
   ...
   * @brief Do pruning
   *
   * Applies the various pruning algorithms, depending on the configuration
   *
   * List of relevant configuration options
   * ======================================
   *
   * @conf{HTauTauPruning, false} brutal chi2 pruning à la Htautau.
   * @conf{CutBasePostfit, false} apply different prunning threshold for norm and shape systs to help for the
   * convergence of the hessian matrix
   *
   * @param c the region in which pruning is applied
   */
  void ApplyPruning(Category& c);
};
