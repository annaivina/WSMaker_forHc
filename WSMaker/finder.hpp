...
class CategoryIndex {
 ...
 public:
  ...
  /**
   * @brief Find all @c Categories matching a given set of properties
   *
   * Used as a replacement of @c RegionTracker: can be called in
   * @c SystematicListsBuilder to find if some regions are present
   *
   * @param pset	The set of properties describing the @c Categories to find
   * @return A set of pointers to the matching @c Categories.
   */
  static std::set<Category*> findAll(const PropertiesSet& pset);

  /**
   * @brief Find all @c Categories matching a given vector of set of properties
   *
   * can be called in @c ComputeNormalizationFactors
   * to find a superset of categories from a vector of properties
   *
   * @param pset      The vector of set of properties describing the @c Categories to find
   * @return A set of pointers to the matching @c Categories.
   */
  static std::set<Category*> findAll(std::vector<PropertiesSet>& pset);
   ...
   * to find a superset of categories from a vector of properties
   *
   * @param cplxCategories  The vector of set of properties functions describing the @c Categories to find
   * @return A set of pointers to the matching @c Categories.
   *
   * For more details on the CategoryFunction structure see systematic.hpp
   */
  static std::set<Category*> findAll(std::vector<ComplexFunctions::CategoryFunction>& cplxCategories);

  /**
   * @brief Check if there is a @c Category matching the given set of properties
   *
   * Used as a replacement of @c RegionTracker: can be called in
   * @c SystematicListsBuilder to find if some regions are present
   *
   * @param pset	The set of properties describing the @c Categories to find
   * @return Whether there are categories matching the requirements
   */
  static bool hasMatching(const PropertiesSet& pset);

  /** @brief Useful list of categories to be used in computeNormalizationFactors
   * @return the whole set of categories in the handler with no selection criteria applied
   */
  static std::set<Category*> GetAllCategoriesSet();
};
