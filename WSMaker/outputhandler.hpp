...
class OutputConfig {
 ...
 public:
   ...
   * @conf{UseGaussianConstraintTypeMCStat,false} Flag to use Gaussian constarints instead of Poissonian
   *
  ...
  /**
   * @brief boolean to have change constraint type to have the matrix convergence
   *
   *  False
   */
  bool doGaussianConstraint;
};
...
class OutputHandler {
 private:
  ...
  /// Whether we have the constrain for CBA to get the convergence or not
  bool m_doGaussianConstraint;
  ...
  /**
   * @brief Add a floating shape to the output
   *
   * Function called by @c addSyst.
   * Hence, no check is performed on the current status, neither on the type
   * of @c sys
   ...
   * @param sys	Systematic to add
   */
  void addShapeFact(const Systematic& sys) const;

  /**
   * @brief Add a MC statistical uncertainty to the output
   *
   * Function called by @c addSyst.
   * Hence, no check is performed on the current status, neither on the type of @c sys
   * Write relative stat error histograms in the output root file, add a ShapeSys line in the XML
   *
   * @param sys       Systematic to add
   */
  void addMCStatSyst(const Systematic& sys) const;
