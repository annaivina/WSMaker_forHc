...
class Plotting {
 private:
  ...
  /// Flag to select if root files are created
  bool m_createAllRootFiles;
 ...
 private:
  ...
  /**
   * @brief Make pie chart plotfor the current category
   *
   * Make pie chart plot. Only backgrounds are plotted.
   *
   * @param c Category to make the plot of
   */
  void makePieChartPlot(Category& c);
