...
class Sample {
 public:
  ...
  using VetoFunction = std::function<bool(const Category&)>;
 ...
 private:
  ...
  /// Can be used to apply vetos categories where this sample will not be present
  VetoFunction m_veto;
 ...
 public:
   ...
   * @param veto function of type `Category -> bool` that can be used to veto this sample in a subset of categories.
   *        The sample will not be added to those categories for which this function return `true`,
   */
  Sample(
      const TString& name, const Type type, const int color,
   ...
   */
  Sample(
      const TString& name, std::vector<TString>&& subsamples, const Type type, const int color,
      VetoFunction&& veto = [](const Category&) { return false; });
  ...
  /**
   * @brief Function called by `engine` to check whether this sample should be vetoed for a certain category.
   *
   * @param c the category under test
   * @return @c true if this sample is to be vetoed
   */
  bool isVetoedIn(Category& c);
  ...
  /**
   * @brief Add a MC statistical uncertainty to this sample
   *
   * Set names of the MC stat systematics, and call @c SamplesInCategory addMCStatSyst
   *
   * @param conf      SysConfig to add
   * @return List of the names of MC stat uncertanties (currently not used)
   */
  std::set<TString> addMCStatSyst(SysConfig& conf);
};
