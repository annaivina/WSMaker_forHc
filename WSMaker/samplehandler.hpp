...
class SampleHandler {
 private:
  ...
  /**
   * @brief Expand indirections in @c m_keywords
   *
   * This allows keywords to refer to other keywords
   * Useful in case of many many samples, e.g HSTXS bins
   */
  void expandKeywords();
 ...
 public:
  ...
  /**
   * @brief Add MC stat uncertainties
   *
   * Loop over samples, and apply MC stat uncertainties to samples that match the given @c SysConfig
   *
   * @param conf      The @c SysConfig to be added
   */
  void addMCStatSyst(SysConfig& conf);
