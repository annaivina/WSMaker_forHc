...
class SampleInCategory {
 ...
 private:
  ...
  std::unique_ptr<Systematic> m_MCStatSys;
   ...
   */
  double doChi2SmoothTest(const SysKey& key, Systematic::Side side, const std::string& opts = "WW CHI2/NDF",
                          bool setErrorsOfSysSmoothedToZero = true) const;
 ...
 public:
   ...
   * * The histograms for MC statistical uncertainty are merged and transported (see below)
   ...
   *
   * The merging of MC statistical uncertainty histograms:
   * There exist two types of histograms that represent MC stat uncertainties.
   * * Systematic::mc_stat_error_absolute_sample is a histogram for the MC stat uncertainties per sample,
   * then they should be summed in quadrature in this function.
   * * Systematic::mc_stat_error_fractional_shared is a histogram for the MC stat uncertainties
   * that have 1-to-1 correspondences to NPs shared in several samples,
   * then they should be just copied here. These histograms are directly passed to the HistoFactory via ShapeSys
   *
   * @param cat Parent category
   * @param s Parent (merged) sample
   * @param sicToMerge List of original @c SampleInCategory that are merged into @c this
   */
  SampleInCategory(Category& cat, Sample& s, const std::vector<SampleInCategory*>& sicToMerge);
   ...
   *
   * Scale a systematic histo wrt the nominal:
   *
   ...
   */
  void smoothHistoRebin(TH1* hsys, SysConfig::Smooth sm);

  /**
   * @brief Smooth a systematic histogram using a kernel smoothing procedure
   *
   * The smoothing is performed on the delta(nominal-systematic)
   *
   * @param  hsys	Systematic histo to smooth
   * @param  sm Smoothing algorithm to use
   */
  void smoothHistoKernel(TH1* hsys, SysConfig::Smooth sm);
   ...
   * @param side      Whether it is the up or down variation of the systematic, or if it is one-sided
   * and should be symmetrized
   * @param treat     If it should be treated norm-only, shape-only, or norm+shape
   * @param hsys      Systematic histogram to deal with
   * @param smooth SysConfig::Smooth enum indicating which kind of smoothing to apply
   * @param symmetrise SysConfig::Symmetrise enum indicating which kind of symmtrisation to apply
   * @param hsys_unsmoothed Add an optional extra histo for shape comparisons before / after syst smoothing
   * @param scaleFactor float indicating a scale factor that the histogram is to be multiplied by
   */
  void addSyst(const TString& sysname, Systematic::Side side, STreat treat, TH1* hsys, SysConfig::Smooth smooth,
               SysConfig::Symmetrise, std::shared_ptr<TH1> hsys_unsmoothed = nullptr, float scaleFactor = 1.0);

  /**
   * @brief Create MC stat uncertainties.
   *
   * This also creates histograms which contain relative errors per bin
   *
   * @param sysname	  Full name of the systematic. All decorrelation tags should already be there
   */
  void addMCStatSyst(const TString& sysname);
   ...
   */
  const Systematic& getSyst(SysType type, const TString& name) const;

  /**
   * @brief Compute the total variation of a syst, including both norm and shape effects
   *
   * Computes the +/-1-sigma variation of the nominal, including both norm and shape
   * effects.
   *
   ...
   */
  TH1* getFullSystShape(const TString& name, Systematic::Side side) const;

  /**
   * @brief Return the reference to the MC statistical uncertainty systematics.
   *
   * @return Unique pointer to @c Systematic for MCstat
   */
  const std::unique_ptr<Systematic>& getMCStatSys() const { return m_MCStatSys; }
  ...
  /**
   * @brief Method to average the up and down shape variation
   *
   * The method averages the up and down shape variation to obtain a symmetric one. It
   * differs from @c symmetriseOneSidedShape in the fact that both up and down are
   * present. [note that the smoothing will be applied after the averaging]
   *
   * @param  sys    the Systematic object
   */
  void averageShape(Systematic& sys);

  /**
   * @brief Method to average the up and down norm variation
   *
   * Average up and down norm variation to yield a symmetric effect
   *
   * @param  sys    the Systematic object
   */
  void averageNorm(Systematic& sys);

  /**
   * @brief Method to symmetrise a one-sided norm syst
   *
   * Invert the effect for the other side of the systematic
   *
   * @param  sys    the Systematic object
   */
  void symmetriseOneSidedShape(Systematic& sys);
   ...
   * @param  sys    the Systematic object
   */
  void symmetriseOneSidedNorm(Systematic& sys);
  ...
  /**
   * @brief Function that calls the smoothing functions
   *
   */
  void smoothSysts();

  /**
   * @brief Function that calls the symmetrisation functions
   *
   */
  void symmetriseAverageSysts();

  /**
   * @brief Function to smooth histogram
   *
   * @param h_up histogram for up variation
   * @param h_do histogram for down variation
   * @param sys Systematic object
   */
  void smoothSysHisto(TH1* h_up, TH1* h_do, const SysConfig::Smooth& sys);
   ...
   */
  void pruneSmallSysts_Yao(TH1* hsig, TH1* hbkg, bool isSensitiveRegion, const std::vector<int>& sensitiveBins);
  ...
  /**
   * @brief Get all names of systematics of a given type in use (passing pruning)
   *
   * @param  type	@c SysType of the systematics to get
   * @return List of names of the systematics of the given type
   */
  const std::set<TString> getUsedSysts(const SysType type) const;
