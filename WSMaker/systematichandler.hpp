...
class SystematicHandler {
 ...
 public:
  ...
  std::vector<TString>& constNormFacts() { return m_constNormFacts; }
  ...
  /// Accessor to MC stat systematics
  std::vector<SysConfig>& MCStatSysts() { return m_MCStatSysts; }
   ...
   */
  void listAllUserSystematics(bool useFltNorms);
  ...
  /* @brief Function to compute the normalization factors of a given systematic to be stored in normalizeRegionsFactors,
   * configured using the normalize() method in systematic.cpp
   *
  ...
  /**@brief Function to retrieve the integral for a given systematics (@sysname) in a given @sample and @category
   * nominal, up and down integrals are conveiently stored in a NormIntegrals structure
   *
   * @param sample   Sample to treat
   * @param cat      Category to treat
   * @param sysname  Name of the systematic to be retrieved (without "up" or "down" suffix) in that particular
   * category (cat) for that particular sample (sample).
   *
   * @return a NormIntegrals object storing nominal, up, down integrals
   * see systematics.hpp for documentation on NormIntegrals
   */
  NormIntegrals GetIntegrals(Sample& sample, Category& cat, TString sysname);
};
