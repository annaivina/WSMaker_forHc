...
void ATLASLabel(Double_t x, Double_t y, const char* text, Color_t color)
{
  ...
  if( text ) {
    TLatex p;
    bool   draw_on_newline = TString(text).BeginsWith("\n");
    double textx = (draw_on_newline ? x : x + delx), texty = (draw_on_newline ? y - 1.05 * l.GetTextSize() : y);
    p.SetNDC();
    p.SetTextFont(42);
    p.SetTextColor(color);
    p.DrawLatex(textx, texty, text);
