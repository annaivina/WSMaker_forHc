...
void drawFCCToys(TString wsName)
{
  ...
  // some config
  TString inFileName = "output/" + wsName + "/fccs/mytoys.root";
  TString plotDir    = "output/" + wsName + "/plots/fccToys/";
  TString gfxExt     = ".png";
  ...
  resultsFile << setw(40) << " " << setw(13) << "parameter" << setw(53) << "parameter_err" << setw(50)
              << "parameter_pull" << setw(50) << "parameter_pull2" << endl;
  resultsFile << setw(40) << " " << setw(8) << "mean" << setw(22) << "width" << setw(19) << "chi2/ndf" << setw(8)
              << "mean" << setw(22) << "width" << setw(19) << "chi2/ndf" << setw(8) << "mean" << setw(22) << "width"
              << setw(19) << "chi2/ndf" << setw(8) << "mean" << setw(22) << "width" << setw(19) << "chi2/ndf" << endl;
  for( int i = 0; i < nPar; i++ ) {
    ...
    for( int err = 0; err <= 3; err++ ) {
      ...
      if( err == 1 )
        drawString = prefix + nameErr;
      if( err == 2 || err == 3 )
        drawString = "(" + prefix + name + "-" + nominal + ")/" + prefix + nameErr;
      drawString += ">>htemp()";
      ...
      // fit
      float delta = 5 * rms;
      TF1*  func  = new TF1("func", "gaus(0)", mean - delta, mean + delta);
      ...
      func->SetLineColor(2);
      func->Draw("same");
      htemp->Fit("func", "", "", mean - delta, mean + delta);
      ...
      // results log
      if( !err ) {
        pullFile << setw(40) << name << setw(12) << TString::Format("%.5f", mean) << "  +/-" << setw(12)
                 << TString::Format("%.5f", rms) << endl;
      }
      if( err == 0 )
        resultsFile << setw(40) << name;
      resultsFile << setw(10) << TString::Format("%.4f", func->GetParameter(1)) << " +/-" << setw(7)
                  << TString::Format("%.4f", func->GetParError(1)) << "  " << setw(8)
                  << TString::Format("%.4f", func->GetParameter(2)) << " +/-" << setw(7)
                  << TString::Format("%.4f", func->GetParError(2)) << "  " << setw(5)
                  << TString::Format("%.2f", func->GetChisquare() / func->GetNDF());
  ...
  for( int i = 0; i < nPar; i++ ) {
    ...
    // for (int j = i; j < nPar; j++) {  // faster scan not possible with drawScatter
    for( int j = 0; j < nPar; j++ ) {
      ...
      // hCorr -> SetBinContent(j + 1, nPar - i, corr); // faster scan not possible with drawScatter

      // draw 2d scatter histograms
      if( drawScatter && nameX.Contains("SigXsec") ) {
        htemp->GetXaxis()->SetTitle(nameX);
        htemp->GetYaxis()->SetTitle(nameY);
        htemp->Draw("colz");
        TString outName     = "corr_" + nameX + "_" + nameY;
        TString outFileName = plotDir + outName + gfxExt;
        c1->Print(outFileName);
      }
      // clean up
