...
void computeFlags(string cardName)
{
  if( !overridden ) {
    ...
    if( cardName.find("2012") != string::npos || cardName.find("12_") != string::npos ||
        cardName.find("2013") != string::npos ) {
      do2012 = true;
...
TGraphAsymmErrors* makeGraphErr(string title, int n, double* x_ary, double* central, double* cenlo, double* cenhi,
...
TLegend* makeLeg2()
{
  ...
  TLegend* leg =
      new TLegend(xmin_leg + xdiff_leg, ymax_leg - ydiff_leg / 2, xmin_leg + 2 * xdiff_leg, ymax_leg, "", "NDC");
  leg->SetFillStyle(0);
  leg->SetTextSize(textSize);
  leg->SetBorderSize(0);
  leg->SetFillColor(0);
...
void save(string baseName, string type, TCanvas* c1)
{
  ...
  string directory(baseName, 0, pos);
  string filename(baseName, pos + 1);
  saveName_saved = baseName;
  system(("mkdir -vp " + directory + "/" + type + "-files").c_str());
  stringstream saveName;
  saveName << directory << "/" << type << "-files/" << filename << "." << type;
  if( doSave )
...
void save()
{
  ...
  doSave           = 1;
  save(saveName_saved, "png", c_saved);
