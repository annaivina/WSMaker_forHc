...
bool drawHatchedBands =
    false; // draw hatched bands around delta muhat = 0 to visualise the total, stat and syst uncertainty
bool drawParamNames = true; // show the nuisance parameter labels
bool drawPostfitImpactBand =
    true; // && (mode != error); // draw a band with the variation of muhat due to each theta (postfit uncertainty)
bool drawPrefitImpactBand =
    false; // && (mode != error); // draw a band with the variation of muhat due to each theta (prefit uncertainty)
...
double  UserlabelSize         = 0.024; // label size
                              ...
                              bool postFitOrder = true)
{
  ...
  if( drawParamNames )
    pad1->SetLeftMargin(0.30); // Graph size
  else
  ...
  TString energy2 = "8";
  TString lumi2   = "20";
  TString lumitex = "#sqrt{s} = " + energy2 + " TeV, #int Ldt = " + lumi2 + " fb^{-1}";
  t3.DrawLatex(channelPosX, channelPosY - 0.02, lumitex.Data());

  TLatex t2;
  t2.SetTextSize(0.025);
  t2.SetNDC();
  t2.DrawLatex(channelPosX, channelPosY - 0.06, ("m_{H}=" + mass + " GeV").c_str());
...
void drawPlot_pulls2(string cardName, string mass, TCanvas* c1, TPad* pad1, TPad* pad2)
{
  ...
  fileHolder pulls_ol;
  if( overlay != "" )
    drawPlot("ascii/" + overlay + "_pulls.txt", 8, pulls_ol);
  ...
  fileHolder nfs_ol;
  if( overlay != "" )
    drawPlot("ascii/" + overlay + "_pulls_nf.txt", 8, nfs_ol);
  ...
  fileHolder cats_ol;
  if( overlay != "" )
    drawPlot("ascii/" + overlay + "_breakdown_add.txt", 3, cats_ol);
  ...
  TGraphAsymmErrors* gr =
      makeGraphErr("", nrNuis, getAry(val), getAry(points_nuis), getAry(down), getAry(up), NULL, NULL);
  ...
  for( int i = 0; i < nrNuis; i++ ) {
    ...
    //        else{
    //            val_norm.push_back(0.);
    //            up_norm.push_back(0.);
    //            down_norm.push_back(0.);
    //        }
  }
  ...
  if( useRelativeImpact )
    ...
    axis_poi->SetTitle("#Delta#hat{#mu}");
  ...
  leg->AddEntry(gr_norm, "Normalization", "lp");
  if( nrNuis_ol > 0 )
  ...
  if( drawPostfitImpactBand ) {
    leg->AddEntry(gr_poi_pos, "+1#sigma Postfit Impact on #hat#mu", "f");
...
TString translateNPname(TString internalName, bool isMVA)
{
  ...
  if( internalName == "alpha_SysBTagB0Effic_Y2012" )
    return "truth b-jet b-tagging efficiency 0";
  if( internalName == "alpha_SysBTagB1Effic_Y2012" )
    return "truth b-jet b-tagging efficiency 1";
  if( internalName == "alpha_SysBTagB2Effic_Y2012" )
    return "truth b-jet b-tagging efficiency 2";
  if( internalName == "alpha_SysBTagB3Effic_Y2012" )
    return "truth b-jet b-tagging efficiency 3";
  if( internalName == "alpha_SysBTagB4Effic_Y2012" )
    return "truth b-jet b-tagging efficiency 4";
  if( internalName == "alpha_SysBTagB5Effic_Y2012" )
    return "truth b-jet b-tagging efficiency 5";
  if( internalName == "alpha_SysBTagB6Effic_Y2012" )
    return "truth b-jet b-tagging efficiency 6";
  if( internalName == "alpha_SysBTagB7Effic_Y2012" )
    return "truth b-jet b-tagging efficiency 7";
  if( internalName == "alpha_SysBTagB8Effic_Y2012" )
    return "truth b-jet b-tagging efficiency 8";
  if( internalName == "alpha_SysBTagB9Effic_Y2012" )
    return "truth b-jet b-tagging efficiency 9";
  if( internalName == "alpha_SysBTagBPythia8_Y2012" )
    return "Generator dependence of b-tagging eff.";
  if( internalName == "alpha_SysBTagBSherpa_Y2012" )
    return "Generator dependence of b-tagging eff.(for Sherpa)";
  if( internalName == "alpha_SysBTagC0Effic_Y2012" )
    return "truth c-jet b-tagging efficiency 0";
  if( internalName == "alpha_SysBTagC1Effic_Y2012" )
    return "truth c-jet b-tagging efficiency 1";
  if( internalName == "alpha_SysBTagC2Effic_Y2012" )
    return "truth c-jet b-tagging efficiency 2";
  if( internalName == "alpha_SysBTagC3Effic_Y2012" )
    return "truth c-jet b-tagging efficiency 3";
  if( internalName == "alpha_SysBTagC4Effic_Y2012" )
    return "truth c-jet b-tagging efficiency 4";
  if( internalName == "alpha_SysBTagC5Effic_Y2012" )
    return "truth c-jet b-tagging efficiency 5";
  if( internalName == "alpha_SysBTagC6Effic_Y2012" )
    return "truth c-jet b-tagging efficiency 6";
  if( internalName == "alpha_SysBTagC7Effic_Y2012" )
    return "truth c-jet b-tagging efficiency 7";
  if( internalName == "alpha_SysBTagC8Effic_Y2012" )
    return "truth c-jet b-tagging efficiency 8";
  if( internalName == "alpha_SysBTagC9Effic_Y2012" )
    return "truth c-jet b-tagging efficiency 9";
  if( internalName == "alpha_SysBTagC10Effic_Y2012" )
    return "truth c-jet b-tagging efficiency 10";
  if( internalName == "alpha_SysBTagC11Effic_Y2012" )
    return "truth c-jet b-tagging efficiency 11";
  if( internalName == "alpha_SysBTagC12Effic_Y2012" )
    return "truth c-jet b-tagging efficiency 12";
  if( internalName == "alpha_SysBTagC13Effic_Y2012" )
    return "truth c-jet b-tagging efficiency 13";
  if( internalName == "alpha_SysBTagC14Effic_Y2012" )
    return "truth c-jet b-tagging efficiency 14";
  if( internalName == "alpha_SysBTagCPythia8_Y2012" )
    return "Generator dependence of b-tagging eff. (for charm)";
  if( internalName == "alpha_SysBTagCSherpa_Y2012" )
    return "Generator dependence of b-tagging eff. (for Sherpa, charm)";
  if( internalName == "alpha_SysBTagL0Effic_Y2012" )
    return "truth l-jet b-tagging efficiency 0";
  if( internalName == "alpha_SysBTagL1Effic_Y2012" )
    return "truth l-jet b-tagging efficiency 1";
  if( internalName == "alpha_SysBTagL2Effic_Y2012" )
    return "truth l-jet b-tagging efficiency 2";
  if( internalName == "alpha_SysBTagL3Effic_Y2012" )
    return "truth l-jet b-tagging efficiency 3";
  if( internalName == "alpha_SysBTagL4Effic_Y2012" )
    return "truth l-jet b-tagging efficiency 4";
  if( internalName == "alpha_SysBTagL5Effic_Y2012" )
    return "truth l-jet b-tagging efficiency 5";
  if( internalName == "alpha_SysBTagL6Effic_Y2012" )
    return "truth l-jet b-tagging efficiency 6";
  if( internalName == "alpha_SysBTagL7Effic_Y2012" )
    return "truth l-jet b-tagging efficiency 7";
  if( internalName == "alpha_SysBTagL8Effic_Y2012" )
    return "truth l-jet b-tagging efficiency 8";
  if( internalName == "alpha_SysBTagL9Effic_Y2012" )
    return "truth l-jet b-tagging efficiency 9";
  ...
  if( internalName == "alpha_SysTChanPtB2" )
    return "Single top t-channel 2nd jet p_{T}";
  ...
  if( internalName == "alpha_SysWMbb_B0_WbbORcc" ) {
    if( isMVA )
      return "W+b#bar{b}, W+c#bar{c} m_{jj} shape (p_{T}^{V} < 120 GeV)";
    else
      return "W+b#bar{b}, W+c#bar{c} m_{jj} shape (p_{T}^{V} < 90 GeV)";
  }
  if( internalName == "alpha_SysWMbb_B1_WbbORcc" )
    return "W+b#bar{b}, W+c#bar{c} m_{jj} shape (90 < p_{T}^{V} < 120 GeV)";
  ...
  if( internalName == "alpha_SysWblWbbRatio" )
    return "W+bl to W+b#bar{b} normalisation (p_{T}^{V} > 120 GeV)";
  if( internalName == "alpha_SysWblWbbRatio_B0" ) {
    if( isMVA )
      return "W+bl to W+b#bar{b} normalisation (p_{T}^{V} < 120 GeV)";
    else
      return "W+bl to W+b#bar{b} normalisation (p_{T}^{V} < 90 GeV)";
  }
  if( internalName == "alpha_SysWblWbbRatio_B1" )
    return "W+bl to W+b#bar{b} normalisation (90 GeV < p_{T}^{V} < 120 GeV)";
  ...
  if( internalName == "alpha_SysZbbNorm_J3" )
    return "Z+b#bar{b} normalisation (3-jet)";
  ...
  if( internalName == "alpha_SysttbarHighPtV" )
    return "ttbar high p_{T}^{V}";
  if( internalName == "alpha_SysttbarNorm_J3" )
    return "ttbar normalization (3-jet)";
  if( internalName == "alpha_SysttbarNorm_J3_L2" )
    return "ttbar normalization (3-jet, dilepton)";
  if( internalName == "alpha_SysMJ_L2_Y2012_Spctopemucr" )
    return "Dilepton Multijet (top C.R.)";
  return internalName;
}
