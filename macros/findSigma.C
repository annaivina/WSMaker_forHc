...
double findSigma(RooAbsReal* nll, double nll_val_true, RooRealVar* par, double val_guess, double val_true, double N,
                 double precision)
{
  ...
  while( fabs(val_pre - val_guess) > precision ) {
    ...
    // loadSnapshot(nll, mu_pre); // do this to avoid comparing multiple minima in the conditional and unconditional
    // fits
...
void setVal(RooRealVar* par, double val)
{
  if( val > 0 && par->getMax() < val )
    par->setMax(2 * val);
  if( val < 0 && par->getMin() > val )
    par->setMin(2 * val);
  par->setVal(val);
}
