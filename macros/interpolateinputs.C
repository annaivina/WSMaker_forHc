...
void compute_interpolated_errors(const std::vector<std::pair<TGraph*, TGraph*>>& bin_errors, TH1* h,
...
void interpolateinputs()
{
  ...
  const vector<pair<TRegexp, vector<double>>> masses_to_interpolate = {
      make_pair("13TeV_TwoLepton_[1-2]+p?tag[0-9]+p?jet_.*_.*ptv_[a-zA-Z]*_mVH.*:AZhllbb",
                vector<double>{/*230., 250.,*/ 270., 290., 310., 330., 350., 370., 390., 410., 430., 450., 470., 490.}),
      make_pair("13TeV_ZeroLepton_[1-2]+p?tag[0-9]+p?jet_.*_.*ptv_[a-zA-Z]*_mVH.*:AZhvvbb",
                vector<double>{310., 330., 350., 370., 390., 410., 430., 450., 470., 490.})};
  ...
  for( size_t j = 0; j < files_to_interpolate.size(); ++j ) {
    ...
    while( (key = (TKey*)nextkey()) ) {
      ...
      if( keyname.Contains("Sys") ) {
        ...
        while( (key2 = (TKey*)nextkey2()) ) {
          ...
          // // FIXME JWH testing
          // if (!origname.Contains("FT_EFF_Eigen_B_0_AntiKt4EMPFlowJets__1up")) continue;
          for( auto& signal_prefix : signal_prefixes ) {
            if( origname.BeginsWith(signal_prefix.c_str()) ) {
              if( check_for_NaNs_and_Infs(h) ) {
                Warning("interpolateinputs",
                        "%s:%s/%s contains a NaN or Inf. Will not include this histogram in the interpolation...",
                        original_full_file.Data(), syst_dir_name.Data(), h->GetName());
                continue;
              }
              if( check_for_all_zeros(h) ) {
                Warning("interpolateinputs",
                        "%s:%s/%s contains all zeros. Will not include this histogram in the interpolation...",
                        original_full_file.Data(), syst_dir_name.Data(), h->GetName());
                continue;
              }
              if( gDebug > 0 )
                Info("interpolateinputs", "\tFound systematic named %s/%s", dirname.Data(), origname.Data());
              ...
              if( !any_of(masses_to_ignore.begin(), masses_to_ignore.end(),
                          [&mass](const double& s) { return s == mass; }) )
        ...
        for( auto& signal_prefix : signal_prefixes ) {
          if( origname.BeginsWith(signal_prefix.c_str()) ) {
            if( check_for_NaNs_and_Infs(h) ) {
              Warning("interpolateinputs",
                      "%s:%s contains a NaN or Inf. Will not include this histogram in the interpolation...",
                      original_full_file.Data(), h->GetName());
              continue;
            }
            if( check_for_all_zeros(h) ) {
              Warning("interpolateinputs",
                      "%s:%s contains all zeros. Will not include this histogram in the interpolation...",
                      original_full_file.Data(), h->GetName());
              continue;
            }
            if( gDebug > 0 )
              Info("interpolateinputs", "\tFound nominal named %s", origname.Data());
            auto mass_name = origname;
            mass_name.ReplaceAll(signal_prefix.c_str(), "");
            auto mass = stod(mass_name.Data());
            if( !any_of(masses_to_ignore.begin(), masses_to_ignore.end(),
    ...
    for( auto& pp : signal_mass_histo_pairs ) {
      ...
      for( int index = 0; static_cast<unsigned>(index) < masses_to_interpolate.size(); ++index )
        if( TString(Form("%s:%s", files_to_interpolate[j].Data(), signal_prefix.c_str()))
                .Contains(masses_to_interpolate[index].first) )
          interpolate_masses_index = index;
      if( interpolate_masses_index == -1 )
        continue;
      ...
      for( auto& p : mass_histo_pairs ) {
        ...
        sort(pairs.begin(), pairs.end(),
             [](const pair<double, TH1*>& i, const pair<double, TH1*>& j) { return (i.first < j.first); });
        auto template_mass_lowest = pairs[0].first, template_mass_highest = pairs.back().first;
        ...
        auto masses = new Double_t[pairs.size()], norms = new Double_t[pairs.size()];
        ...
        for( unsigned i = 0; i < masses_to_interp.size(); ++i ) {
          if( gDebug > 5 )
            Info("interpolateinputs", "\tProcessing interpolated histogram %s",
                 Form("%s%d%s", signal_prefix.c_str(), int(masses_to_interp[i]), tag.c_str()));
          TH1*    h = nullptr;
          TString h_name;
          if( template_mass_lowest > masses_to_interp[i] || template_mass_highest < masses_to_interp[i] ) {
            ...
            h = morph.createHistogram(Form("%s%d%s", signal_prefix.c_str(), int(masses_to_interp[i]), tag.c_str()), m,
        ...
        for( auto& p : histPdfs )
          delete p;
        for( auto& p : dataHists )
          delete p;
        for( auto& p : bin_errors ) {
          delete p.first;
          delete p.second;
        }
    ...
    if( was_interpolated ) {
      if( gDebug > 0 )
        Info("interpolateinputs", "\tCreating plots...");
      ...
      for( auto& pp : signal_mass_histo_pairs ) {
        ...
        for( auto& p : mass_histo_pairs ) {
          ...
          auto canvas_hist = new TCanvas(Form("%s%s_signal_histograms", signal_prefix.c_str(), tag.c_str()),
                                         "Signal Interpolation", 600, 600);
          auto canvas_pdf  = new TCanvas(Form("%s%s_signal_pdfs", signal_prefix.c_str(), tag.c_str()),
                                         "Signal Interpolation", 600, 600);
          bool is_first    = true;
          for( auto& ps : pairs ) {
            auto mass = ps.first;
            ...
            bool is_interpolated = any_of(masses_to_interp.begin(), masses_to_interp.end(),
                                          [&mass](const double& m) { return mass == m; });
                                      ...
                                      const Double_t*                             masses)
{
  ...
  int  bin_counter = 0;
  auto left_exit = false, right_exit = false;

  while( !left_exit || !right_exit ) {
    left_exit    = true;
    right_exit   = true;
    auto left_up = new Double_t[mass_histo_pairs.size()], left_down = new Double_t[mass_histo_pairs.size()],
         right_up = new Double_t[mass_histo_pairs.size()], right_down = new Double_t[mass_histo_pairs.size()];
                                 ...
                                 const double& mass_to_interpolate)
{
  ...
  while( !left_exit || !right_exit ) {
    ...
    if( bin_counter == 0 ) {
      ...
      h->SetBinError(center_bin, 0.5 * (up_error_graph->Eval(mass_to_interpolate, 0, "S") +
                                        down_error_graph->Eval(mass_to_interpolate, 0, "S")));
      // h->SetBinError(center_bin, 0.5*(up_error_graph->Eval(mass_to_interpolate, &spline, "S") +
      // down_error_graph->Eval(mass_to_interpolate, &spline, "S")));
      ...
      auto right_bin              = center_bin + bin_counter;
      if( left_bin >= 1 ) {
        h->SetBinError(left_bin, 0.5 * (up_error_graph_left->Eval(mass_to_interpolate, 0, "S") +
                                        down_error_graph_left->Eval(mass_to_interpolate, 0, "S")));
        // h->SetBinError(left_bin, 0.5*(up_error_graph_left->Eval(mass_to_interpolate, &spline, "S") +
        // down_error_graph_left->Eval(mass_to_interpolate, &spline, "S")));
        left_exit = false;
      }
      if( right_bin <= h->GetNbinsX() ) {
        h->SetBinError(right_bin, 0.5 * (up_error_graph_right->Eval(mass_to_interpolate, 0, "S") +
                                         down_error_graph_right->Eval(mass_to_interpolate, 0, "S")));
        // h->SetBinError(right_bin, 0.5*(up_error_graph_right->Eval(mass_to_interpolate, &spline, "S") +
        // down_error_graph_right->Eval(mass_to_interpolate, &spline, "S")));
        right_exit = false;
      }
    }
...
{
  for( Int_t ibin = 1; ibin <= h->GetNbinsX(); ++ibin ) {
    if( !TMath::AreEqualRel(h->GetBinContent(ibin), 0.0, 1.0e-12) )
      return false;
  }
  return true;
}
