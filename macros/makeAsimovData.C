...
RooDataSet* makeAsimovData(ModelConfig* mcInWs, bool doConditional, RooWorkspace* combWS, RooAbsPdf* combPdf,
                           RooDataSet* combData, double mu_asimov)
{
  ...
  TIterator*  it = mcInWs->GetParametersOfInterest()->createIterator();
  ...
  it = mcInWs->GetParametersOfInterest()->createIterator();
  n  = (RooRealVar*)it->Next();
  while( n ) {
    n->setVal(mu_asimov);
    n->setConstant(1);
  ...
  for( int i = 0; i < nrNuis; i++ ) {
    ...
    cout << "Setting glob: " << glob->GetName() << ", which had previous val: " << glob->getVal()
  ...
  if( combWS->var("ATLAS_dummyX") )
    combWS->var("ATLAS_dummyX")->setBins(1);
  ...
  RooCategory* channelCat =
      (RooCategory*)&simPdf
          ->indexCat(); //(RooCategory*)combWS->cat("master_channel");//(RooCategory*) (&simPdf->indexCat());
  ...
  RooDataSet* asimovData = new RooDataSet(("asimovData" + muStr.str()).c_str(), ("asimovData" + muStr.str()).c_str(),
