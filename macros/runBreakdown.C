...
void setParams(RooArgSet* nuis, list<string> tmp_list, string technique, RooFitResult* fitresult, RooRealVar* poi,
               double corrCutoff);
...
void runBreakdown(string inFileName = "data_newbug.root",
                  string wsName = "workspace", string modelConfigName = "modelSB", string dataName = "data",
                  string xmlName = "config/breakdown.xml", string technique = "add", string catecory2eval = "total",
                  double precision = 0.005, double corrCutoff = 0.0, string folder = "data_newbug",
                  string loglevel = "DEBUG")
{
  ...
  */
  ...
  while( RooRealVar* poi = (RooRealVar*)POI_itr->Next() ) {
    if( !poi ) {
      // LOG(logERROR) << "POI: " << parsed[i] << " doesn't exist!";
      LOG(logERROR) << "POI: doesn't exist!";
      exit(1);
    }
    LOG(logINFO) << "Find POI " << poi->GetName();
    poi->setVal(1);
    if( anaType == "VHccRun2" ) {
      poi->setRange(-30., 30.);
    } else {
  ...
  if( config->catecory2eval == "total" ) {
    for( unsigned int i = 0; i < pois.size(); i++ ) {
      ...
      pois_up.push_back(findSigma(nll, nll_val_true, pois[i], pois_hat[i] + pois[i]->getErrorHi(), pois_hat[i], +1,
                                  config->precision));
      ws->loadSnapshot("tmp_shot");
      pois_down.push_back(findSigma(nll, nll_val_true, pois[i], pois_hat[i] - fabs(pois[i]->getErrorLo()), pois_hat[i],
                                    -1, config->precision));
    }
  ...
  for( unsigned int i = 0; i < pois.size(); i++ ) {
    LOG(logINFO) << config->catecory2eval << " gives " << pois[i]->GetName() << " = " << pois_hat[i] << " +"
                 << pois_up[i] << " / -" << pois_down[i];
  }

  system(("mkdir -vp output/" + string(config->folder) + "/root-files/breakdown_" + string(technique)).c_str());
  stringstream fileName;
  fileName << "output/" << config->folder << "/root-files/breakdown_" << technique << "/" << config->catecory2eval
           << ".root";
  TFile fout(fileName.str().c_str(), "recreate");

  TH1D* h_out =
      new TH1D(config->catecory2eval.c_str(), config->catecory2eval.c_str(), 3 * pois.size(), 0, 3 * pois.size());
      ...
      while( systematic != 0 ) {
        ...
        while( attr_syst != 0 ) {
          ...
          if( breakdown ) {
            ...
            runBreakdown(config->inFileName, config->wsName, config->modelConfigName, config->dataName,
                         "config/tmp_" + systName + ".xml", config->technique, systName, config->precision,
                         config->corrCutoff, config->folder, config->loglevel);
          }
