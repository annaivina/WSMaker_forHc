...
bool DetermineSmearFromFileName(const TString& name) { return (name.Contains("TwoLepton") && name.Contains("SR")); }
...
void smearinputs(int percent_max = 5, int smear = 5)
{
  ...
  for( size_t i = 0; i < smear_percents.size(); ++i ) {
    ...
    for( size_t j = 0; j < files_to_copy.size(); ++j ) {
      TString file_to_copy = src_dir + "/" + files_to_copy[j], destination_file = new_dir + "/" + files_to_copy[j];
      if( print_lvl >= 1 )
        cout << "Copying input: " << destination_file.Data() << endl;
    ...
    for( size_t j = 0; j < files_to_smear.size(); ++j ) {
      ...
      if( print_lvl >= 1 )
        cout << "Creating newly smeared input: " << new_full_file.Data() << endl;
      ...
      while( (key = (TKey*)nextkey()) ) {
        ...
        if( key && keyname.Contains("Sys") ) {
          ...
          while( (key2 = (TKey*)nextkey2()) ) {
            ...
            if( origname.Contains(signal) )
              h = smearsignal(h, smearf, origname, signal);
...
TH1* smearhisto(TH1* orighisto, float mass, float smear)
{
  ...
  // smearedhisto->Sumw2();
  if( smearedhisto->Integral() != 0 )
    smearedhisto->Scale(orighisto->Integral() / smearedhisto->Integral());
