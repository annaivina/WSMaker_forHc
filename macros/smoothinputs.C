...
//                  comma-separated list of file name keyword(s) to logically "and" together in order to include to
//                      determining whether to smooth or not
//                  comma-separated list of file name keyword(s) to logically "or" together in order to exclude in
//                      determining whether to smooth or not
//                  bool that is true if the full histogram range is left untouched on left side (false to truncate
//                      at first bin with positive data) - default = true
//                  bool that is true if the full histogram range is left untouched on right side (false to truncate
//                      at last bin with positive data) - default = true
//                  double representing a KNN (in fraction of events) that scales the bandwidth (never decreases)
//                      - default = 0.0
//                  double representing an event factor that scales the bandwidth (increase for narrower bandwidths
//                      and decrease for larger bandwidths) - default = 1.0
//                  double representing the KNN parameter fractional variation (used in creating a systematic
//                      associated with smoothing) - default = 0.05
//                  double representing the event-factor parameter fractional variation (used in creating a systematic
//                      associated with smoothing) - default = 0.05
...
//  root -l -q -b "smoothinputs.C+(\"/Users/jhetherly/Documents/Graduate_Work/ATLAS/HSG5/Zbb_Smoothing/21009\",
//  {\"Zcc,Zbc,Zbb;TwoLepton,SR;fat;false;true;0.005;1.0\"})"

bool check_for_NaNs_and_Infs(const TH1* h);

bool DetermineSmoothFromFileName(const TString& name, const std::vector<std::string>& all_include,
                                 const std::vector<std::string>& any_exclude)
{
  bool result = true;
  for( auto& s : all_include )
    result &= name.Contains(s.c_str());
  if( result )
    for( auto& s : any_exclude )
      result &= !name.Contains(s.c_str());
  return result;
}
...
// function that controls the smoothing - this histogram passed in is deleted and TH1* returned assumes it's name and
// title
std::pair<TH1*, TH2*> smooth_histo(TH1*, const double&, const double&, const bool&, const bool&,
                                   const std::string& = "lin", const double& = 1.0, const bool& = false);
...
void smoothinputs(const TString& src_dir, const std::vector<std::string>& settings,
                  const bool& write_new_inputs = false)
{
  ...
  for( decltype(n_samples) isetting = 0; isetting < n_samples; ++isetting ) {
    ...
    if( tokens[0].size() != 0 ) {
      ...
      for( auto& s : samples_to_smooth.back() )
        combined_samples_to_smooth.back() += s;
    ...
    if( tokens.size() >= 4 )
      full_low_ranges.push_back((tokens[3] == "true" || tokens[3] == ""));
    else
      full_low_ranges.push_back(true);

    if( tokens.size() >= 5 )
      full_high_ranges.push_back((tokens[4] == "true" || tokens[4] == ""));
    else
      full_high_ranges.push_back(true);

    if( tokens.size() >= 6 )
      knn_parameters.push_back((tokens[5] != "") ? stod(tokens[5]) : 0.0);
    else
      knn_parameters.push_back(0.0);
    if( knn_parameters.back() > 1.0 || knn_parameters.back() < 0.0 )
      knn_parameters.back() = 0.0;

    if( tokens.size() >= 7 )
      ef_parameters.push_back((tokens[6] != "") ? stod(tokens[6]) : 1.0);
    else
      ef_parameters.push_back(1.0);

    if( tokens.size() >= 8 )
      knn_variation_factors.push_back((tokens[7] != "") ? stod(tokens[7]) : 0.0);
    else
      knn_variation_factors.push_back(0.0);
    if( knn_variation_factors.back() >= 1.0 || knn_variation_factors.back() < 0.0 )
      knn_variation_factors.back() = 0.0;

    if( tokens.size() >= 9 )
      ef_variation_factors.push_back((tokens[8] != "") ? stod(tokens[8]) : 0.0);
    else
      ef_variation_factors.push_back(0.0);
    if( ef_variation_factors.back() >= 1.0 || ef_variation_factors.back() < 0.0 )
      ef_variation_factors.back() = 0.0;

    if( tokens.size() >= 10 )
      scales.push_back((tokens[9] != "") ? tokens[9] : "lin");
    else
      scales.push_back("lin");

    if( tokens.size() >= 11 )
      grid_factors.push_back((tokens[10] != "") ? stod(tokens[10]) : 1.0);
    else
      grid_factors.push_back(1.0);
  ...
  // determine which files to smooth and which to ignore
  if( files ) {
    TSystemFile* file;
    TString      fname;
    ...
    while( (file = (TSystemFile*)next()) ) {
      fname = file->GetName();
      if( !file->IsDirectory() && fname.EndsWith(extension.Data()) && !fname.Contains("ORIGINAL_") &&
          !fname.Contains("TEMP_") ) {
        bool should_smooth = false;
        for( decltype(n_samples) isetting = 0; isetting < n_samples; ++isetting )
          should_smooth |=
              DetermineSmoothFromFileName(fname, catagories_all_include[isetting], catagories_any_exclude[isetting]);
  ...
  for( size_t j = 0; j < files_to_smooth.size(); ++j ) {
    ...
    TString original_file         = original_full_file(original_full_file.Last('/') + 1,
                                                       original_full_file.Length() - (original_full_file.Last('/') + 1));
    TString new_full_file         = new_dir + "/" + original_file;
    TString new_full_file_control = new_dir_control + "/" + original_file;
    if( gDebug >= 0 && write_new_inputs )
      Info("smoothinputs", "Creating newly combined input %s", new_full_file.Data());
    ...
    while( (key = (TKey*)nextkey()) ) {
      ...
      if( key && keyname.Contains("Sys") ) {
        if( systematics_dir_name == "" )
          systematics_dir_name = keyname;
        ...
        while( (key2 = (TKey*)nextkey2()) ) {
          ...
          for( decltype(n_samples) isetting = 0; isetting < n_samples; ++isetting )
            if( syst_histos[isetting].count(systname.Data()) == 0 )
              syst_histos[isetting][systname.Data()] = nullptr;
          bool                is_histo  = false;
          decltype(n_samples) iisetting = 0;
          for( decltype(n_samples) isetting = 0; isetting < n_samples; ++isetting ) {
            if( !DetermineSmoothFromFileName(_file0->GetName(), catagories_all_include[isetting],
                                             catagories_any_exclude[isetting]) )
              continue;
        ...
        for( decltype(n_samples) isetting = 0; isetting < n_samples; ++isetting ) {
          for( it_type iterator = syst_histos[isetting].begin(); iterator != syst_histos[isetting].end(); ++iterator ) {
            ...
            if( h_to_smooth != 0 ) {
              ...
              // }
              h_to_smooth->SetName(
                  hist_name.replace(0, hist_name.find_first_of('_'), combined_samples_to_smooth[i_sample]).c_str());
              h_to_smooth->SetTitle(
                  hist_title.replace(0, hist_title.find_first_of('_'), combined_samples_to_smooth[i_sample]).c_str());

              TH1* smoothed_histo = 0;
              if( DetermineSmoothFromFileName(_file0->GetName(), catagories_all_include[i_sample],
                                              catagories_any_exclude[i_sample]) &&
                  !check_for_NaNs_and_Infs(h_to_smooth) ) {
                _file1->cd(dirname);
                if( gDebug >= 0 )
                  Info("smoothinputs", "Smoothing histogram %s", h_to_smooth->GetName());
                smoothed_histo = smooth_histo(h_to_smooth, knn_parameters[i_sample], ef_parameters[i_sample],
                                              full_low_ranges[i_sample], full_high_ranges[i_sample], scales[i_sample],
                                              grid_factors[i_sample])
                                     .first;
              } else {
    ...
    if( static_cast<decltype(nominal_hists_to_smooth.size())>(
            count(nominal_hists_to_smooth.begin(), nominal_hists_to_smooth.end(), nullptr)) !=
        nominal_hists_to_smooth.size() ) {
      for( decltype(n_samples) isetting = 0; isetting < n_samples; ++isetting ) {
        if( nominal_hists_to_smooth[isetting] == nullptr )
          continue;
        ...
        nominal_hists_to_smooth[isetting]->SetTitle(
            hist_title.replace(0, hist_title.find_first_of('_'), combined_samples_to_smooth[isetting]).c_str());
        TH1 * smoothed_histo = nullptr, *smoothed_histo_syst_up = nullptr, *smoothed_histo_syst_down = nullptr;
        ...
        if( DetermineSmoothFromFileName(_file0->GetName(), catagories_all_include[isetting],
          ...
          if( gDebug >= 0 )
            Info("smoothinputs", "Smoothing histogram %s", nominal_hists_to_smooth[isetting]->GetName());
          ...
          auto smoothed_result      = smooth_histo(
                   nominal_hists_to_smooth[isetting], knn_parameters[isetting], ef_parameters[isetting],
                   full_low_ranges[isetting], full_high_ranges[isetting], scales[isetting], grid_factors[isetting], true);
          ...
          // std::pair<double, double> knn_variations(knn_parameters[isetting]*(1.0 - knn_variation_factors[isetting]),
          // knn_parameters[isetting]*(1.0 + knn_variation_factors[isetting])),
          //                           ef_variations(ef_parameters[isetting]*(1.0 - ef_variation_factors[isetting]),
          //                           ef_parameters[isetting]*(1.0 + ef_variation_factors[isetting]));
          std::pair<double, double> knn_variations(
              TMath::Max(knn_parameters[isetting] - knn_variation_factors[isetting], 0.0),
              TMath::Min(knn_parameters[isetting] + knn_variation_factors[isetting], 0.999)),
              ef_variations(ef_parameters[isetting] * (TMath::Power(10.0, -10.0 * ef_variation_factors[isetting])),
                            ef_parameters[isetting] * (TMath::Power(10.0, 10.0 * ef_variation_factors[isetting])));
          auto smoothed_histo_syst_down_down =
              smooth_histo(nominal_down_down, knn_variations.first, ef_variations.first, full_low_ranges[isetting],
                           full_high_ranges[isetting], scales[isetting], grid_factors[isetting])
                  .first;
          auto smoothed_histo_syst_down_up =
              smooth_histo(nominal_down_up, knn_variations.first, ef_variations.second, full_low_ranges[isetting],
                           full_high_ranges[isetting], scales[isetting], grid_factors[isetting])
                  .first;
          auto smoothed_histo_syst_up_down =
              smooth_histo(nominal_up_down, knn_variations.second, ef_variations.first, full_low_ranges[isetting],
                           full_high_ranges[isetting], scales[isetting], grid_factors[isetting])
                  .first;
          auto smoothed_histo_syst_up_up =
              smooth_histo(nominal_up_up, knn_variations.second, ef_variations.second, full_low_ranges[isetting],
                           full_high_ranges[isetting], scales[isetting], grid_factors[isetting])
                  .first;
          ...
          for( Int_t ibin = 1; ibin <= smoothed_histo->GetXaxis()->GetNbins(); ++ibin ) {
            ...
            if( nominal_bin_content != 0.0 ) {
              smoothed_histo_syst_down_down_delta->SetBinContent(ibin, (down_down_bin_content - nominal_bin_content) /
                                                                           nominal_bin_content);
              smoothed_histo_syst_down_up_delta->SetBinContent(ibin, (down_up_bin_content - nominal_bin_content) /
                                                                         nominal_bin_content);
              smoothed_histo_syst_up_down_delta->SetBinContent(ibin, (up_down_bin_content - nominal_bin_content) /
                                                                         nominal_bin_content);
              smoothed_histo_syst_up_up_delta->SetBinContent(ibin, (up_up_bin_content - nominal_bin_content) /
                                                                       nominal_bin_content);
            } else {
          ...
          bool smoothed_histo_syst_down_down_keep = false, smoothed_histo_syst_down_up_keep = false,
               smoothed_histo_syst_up_down_keep = false, smoothed_histo_syst_up_up_keep = false;
          // find up variation
          ...
          smoothed_histo_syst_up->SetName(
              Form("%s_SysWSMaker_Smoothing__1up", combined_samples_to_smooth[isetting].c_str()));
          if( smoothed_histo_syst_up == smoothed_histo_syst_down )
            smoothed_histo_syst_down = (TH1*)smoothed_histo_syst_up->Clone();
          smoothed_histo_syst_down->SetName(
              Form("%s_SysWSMaker_Smoothing__1down", combined_samples_to_smooth[isetting].c_str()));
          bool up_nominal = true, down_nominal = true;
          for( Int_t ibin = 1; ibin <= smoothed_histo_syst_up->GetXaxis()->GetNbins(); ++ibin ) {
            if( !up_nominal && !down_nominal )
              break;
          ...
          if( corr != nullptr ) {
            ...
            xbins[smoothed_histo->GetXaxis()->GetNbins()] =
                smoothed_histo->GetXaxis()->GetBinUpEdge(smoothed_histo->GetXaxis()->GetNbins());
            rebinned_corr =
                new TH2D(Form("%s_correlation_histogram", smoothed_histo->GetName()), "",
                         smoothed_histo->GetXaxis()->GetNbins(), xbins, smoothed_histo->GetXaxis()->GetNbins(), xbins);
        ...
        if( smoothed_histo != nullptr && rebinned_corr != nullptr && smoothed_histo_syst_up != nullptr &&
    ...
    _file0->Close();
    _file1->Close();
    for( auto& h : nominal_hists_to_smooth )
      h = nullptr;

    // _file2 = (smoothed_nominal_histo != nullptr && unsmoothed_nominal_histo != nullptr) ?
    // TFile::Open(new_full_file_control.Data(), "RECREATE") : nullptr;
    _file2 = (static_cast<decltype(smoothed_nominal_histos.size())>(
                  count(smoothed_nominal_histos.begin(), smoothed_nominal_histos.end(), nullptr)) !=
                  smoothed_nominal_histos.size() &&
              static_cast<decltype(unsmoothed_nominal_histos.size())>(
                  count(unsmoothed_nominal_histos.begin(), unsmoothed_nominal_histos.end(), nullptr)) !=
                  unsmoothed_nominal_histos.size())
                 ? TFile::Open(new_full_file_control.Data(), "RECREATE")
                 : nullptr;

    if( _file2 != nullptr ) {
      _file2->mkdir(dirname, dirname);
      if( gDebug >= 0 )
        Info("smoothinputs", "Making plots file: %s.", _file2->GetName());
      if( write_new_inputs ) {
        _file0 = TFile::Open(copied_file_name.Data());
      ...
      while( (key = (TKey*)nextkey()) ) {
        ...
        if( key && keyname.Contains("Sys") ) {
          ...
          typedef map<string, TH1*>::iterator it_type;
          for( decltype(n_samples) isetting = 0; isetting < n_samples; ++isetting ) {
            for( it_type iterator = syst_histos[isetting].begin(); iterator != syst_histos[isetting].end();
                 ++iterator ) {
              TH1* h_to_smooth = iterator->second;
              if( h_to_smooth != nullptr ) {
                ...
                // }
                string smoothed_hist_name(
                    hist_name.replace(0, hist_name.find_first_of('_'), combined_samples_to_smooth[i_sample]));
                ...
                // Spyros : write histos instead of canvas
                smoothed_delta->Write();

                /*auto canvas = new TCanvas(smoothed_hist_name.c_str(), Form("%s #Delta comparison",
                smoothed_histo->GetName()), 600, 600); unsmoothed_delta->SetTitle(Form("%s #Delta comparison",
                smoothed_hist_name.c_str())); unsmoothed_delta->SetStats(kFALSE);
          ...
          _file2->cd(dirname);
          for( decltype(n_samples) isetting = 0; isetting < n_samples; ++isetting ) {
            if( smoothed_nominal_histos[isetting] == nullptr || smoothed_nominal_histos_syst_up[isetting] == nullptr )
              continue;
            auto smoothed_histo    = smoothed_nominal_histos[isetting];
            auto smoothed_histo_up = smoothed_nominal_histos_syst_up[isetting];
            // string smoothed_hist_name(hist_name.replace(0, hist_name.find_first_of('_'),
            // combined_samples_to_smooth[i_sample]));

            auto smoothed_delta_up = (TH1*)smoothed_histo->Clone();

            smoothed_delta_up->Add(smoothed_histo_up, -1.0);

            auto canvas = new TCanvas(smoothed_histo_up->GetName(),
                                      Form("%s #Delta comparison", smoothed_histo->GetName()), 600, 600);
            ...
            canvas = new TCanvas(smoothed_histo_down->GetName(),
                                 Form("%s #Delta comparison", smoothed_histo->GetName()), 600, 600);
      ...
      // Spyros : write root files instead of canvas

      auto canvas = new TCanvas("nominal_smoothing_comp", "Unsmoothed vs. smoothed nominal", 600, 600);
      for( decltype(n_samples) isetting = 0; isetting < n_samples; ++isetting ) {
        if( unsmoothed_nominal_histos[isetting] == nullptr )
          continue;

        smoothed_nominal_histos[isetting]->Write();
        ...
        if (smoothed_nominal_corrs[isetting] != nullptr) {
          canvas->Clear();
    ...
    namespace details_
{
  ...
  template <class T> struct BigAllocator /*: public std::allocator<T>*/ {
   public:
    using value_type      = T;
    using difference_type = long long;
    using size_type       = unsigned long long;
    BigAllocator()        = default;
    template <class U> BigAllocator(const BigAllocator<U>&) {}
  ...
  template <typename T, typename U> inline bool operator==(const BigAllocator<T>&, const BigAllocator<U>&)
  {
    return true;
  }

  template <typename T, typename U> inline bool operator!=(const BigAllocator<T>& a, const BigAllocator<U>& b)
  {
    return !(a == b);
  }
  ...
  // inner loop
  template <unsigned J> struct GetIndex_impl_impl {
    template <unsigned DIM, typename T, typename U> static void apply(T& p, const U& ngp)
    {
      p *= std::get<DIM - J>(ngp);
  ...
  template <> struct GetIndex_impl_impl<1> {
    template <unsigned DIM, typename T, typename U> static void apply(T& p, const U& ngp)
    {
      p *= std::get<DIM - 1>(ngp);
    }
  ...
  // outer loop
  template <unsigned I> struct GetIndex_impl {
    template <unsigned DIM, typename T, typename U> static void apply(T& r, const U& indices, const U& ngp)
    {
      T product = 1;
  ...
  template <> struct GetIndex_impl<1> {
    template <unsigned DIM, typename T, typename U> static void apply(T& r, const U& indices, const U& /*ngp*/)
    {
  ...
  template <unsigned I> struct GetIndices_impl {
    template <unsigned DIM, typename T, typename U, typename V> static void apply(T& r, const U& index, const V& ngp)
    {
  ...
  template <> struct GetIndices_impl<0> {
    template <unsigned DIM, typename T, typename U, typename V> static void apply(T& r, const U& index, const V& ngp)
    {
...
template <unsigned DIMENSION = 1> class DensityEstimator {
  ...
  bool                                m_use_adaptive_bandwidth = true, m_full_correlations = false;
  double                              m_normalization = 1.0;
  DimArray<int>                       m_number_of_grid_points;
  SparceCoordList<DimArray<double>>   m_xs;
  SparceCoordList<double>             m_ws, m_ues, m_des;
  DimArray<std::pair<double, double>> m_bounds, m_selection_bounds;
  DimArray<double>                    m_default_hs, m_taus, m_variance_fractions, m_event_factors;
  DimArray<TF1>                       m_scaling_funcs, m_scaling_func_derivatives, m_inverse_scaling_funcs, m_kernels;
  ...
  int KNNDistance(const BigVector<double>& c, const double& sum_c, const int& i, const unsigned& dim, const double& d,
                  const KNNType& t) const
  {
    ...
    if( t == KNNType::Standard ) {
      int    left_index = i - 1, right_index = i + 1;
      double left_sum = dist_sum, right_sum = dist_sum;
      auto   left_distance = 0, right_distance = 0;
      ...
      while( dist_sum > 0.0 ) {
        ...
        bool first_block = (left_sum >= right_sum), second_block = (left_sum <= right_sum);
        if( first_block ) {
  ...
  {
    double result = 0;
    double _ci2 = 0, _c = 0, _ci = 0, _c2 = 0;
  ...
  {
    if( i >= 0 && i < m_number_of_grid_points[dim] )
      return i;
    if( i < 0 && i >= -m_number_of_grid_points[dim] )
      return -i - 1;
    if( i >= m_number_of_grid_points[dim] && i < 2 * m_number_of_grid_points[dim] )
      return m_number_of_grid_points[dim] - 1 - i % m_number_of_grid_points[dim];
    Error("DensityEstimator::GetWrappedIndex", "Index %d is out of wrapped range", i);
    return -1;
  }

  BigVector<std::pair<CoordIndex, double>>
  ComputeGridOfKernelEvaluations(const CoordIndex& index, const DimArray<SparceCoordList<int>>& Ls,
      ...
      for( unsigned repeat_delta = 0; repeat_delta < total_storage; repeat_delta += (2 * L + 1) * dimension_delta ) {
        ...
        for( typename std::make_signed<CoordIndex>::type l = -L; l <= L; ++l ) {
          ...
          for( CoordIndex i = 0; i < dimension_delta; ++i ) {
            ...
            if( m_use_adaptive_bandwidth ) {
              if( dim == 0 )
                result_[counter + repeat_delta].second = ks[dim].at(indices[dim])[l < 0 ? -l : l];
              else
                result_[counter + repeat_delta].second *= ks[dim].at(indices[dim])[l < 0 ? -l : l];
            } else {
              if( dim == 0 )
                result_[counter + repeat_delta].second = ks[dim].at(0)[l < 0 ? -l : l];
              else
                result_[counter + repeat_delta].second *= ks[dim].at(0)[l < 0 ? -l : l];
            }
            ++counter;
  ...
  {
    auto k_indices = this->GetIndices(k), l_indices = this->GetIndices(l);

    for( unsigned dim = 0; dim < DIMENSION; ++dim )
      if( TMath::Abs(k_indices[dim] - l_indices[dim]) > 1 )
        return false;
    return true;
  }

  void ComputeSmoothedValues(BigVector<double>& fs, const DimArray<SparceCoordList<int>>& Ls,
                             const DimArray<SparceCoordList<std::vector<double>>>& ks, const decltype(m_ws)& c,
                             const decltype(m_bounds)& extremes, const DimArray<double>& deltas) const
  {
    ...
    if( m_use_adaptive_bandwidth ) {
      ...
      for( CoordIndex j = 0; j < fs.size(); ++j ) {
        ...
        for( unsigned dim = 0; dim < DIMENSION; ++dim )
          reverse_scale *=
              m_scaling_func_derivatives[dim](m_inverse_scaling_funcs[dim](extremes[dim].first + j * deltas[dim]));
        fs[j] *= reverse_scale;
      }
    } else {
  ...
  void ComputeSmoothedValuesAndErrors(
      ...
      const decltype(m_bounds)& extremes, const DimArray<double>& deltas) const
  {
    ...
    for( const auto& p : c ) {
      ...
      const auto& weight = p.second;
      // skip negative weights and errors
      if( weight <= 0.0 || c_error.at(index) <= 0.0 )
        continue;
      auto kernel_grid = ComputeGridOfKernelEvaluations(index, Ls, ks);
      for( auto& pp : kernel_grid ) {
        auto this_index  = pp.first;
    ...
    if( m_full_correlations )
      Info("DensityEstimator::ComputeSmoothedValuesAndErrors", "Computing full covariance matrix (patience)");
    for( CoordIndex i = 0; i < fs_cov.size(); ++i ) {
      ...
      for( CoordIndex j = i; j < j_limit; ++j ) {
        for( typename std::make_signed<CoordIndex>::type k = 0; static_cast<CoordIndex>(k) < fs_a.size(); ++k ) {
          if( c_error.count(k) == 0 )
            continue;
          double cek = c_error.at(k);
          if( cek <= 0.0 || fs_a[i][k] <= 0.0 )
            continue;
          for( typename std::make_signed<CoordIndex>::type l = 0; static_cast<CoordIndex>(l) < fs_a[k].size(); ++l ) {
            if( c_error.count(l) == 0 || !this->AreNeighbors(k, l) )
              continue; // covariance matrix is almost diagonal
  ...
  bool BinningEqual(const TH1* h1, const TH1* h2) const
  {
    ...
    for( int ibin = 1; ibin <= h1->GetXaxis()->GetNbins(); ++ibin ) {
      if( h1->GetXaxis()->GetBinCenter(ibin) != h2->GetXaxis()->GetBinCenter(ibin) )
        return false;
      if( h1->GetXaxis()->GetBinUpEdge(ibin) != h2->GetXaxis()->GetBinUpEdge(ibin) )
        return false;
      if( h1->GetXaxis()->GetBinLowEdge(ibin) != h2->GetXaxis()->GetBinLowEdge(ibin) )
        return false;
    }
    return true;
  }
  ...
  // useful for FFT (global bandwidth)
  void ComplexAdd(const double& a, const double& b, const double& c, const double& d, double& re, double& im) const
  {
    double k1 = a * (c + d), k2 = d * (a + b), k3 = c * (b - a);
    re = k1 - k2;
    im = k1 + k3;
  }
  ...
  TH1* MakeCorrelationHistogram(const BigVector<BigVector<double>>& final_cov) const
  {
    ...
    auto*      result  = new TH2D(Form("DensityEstimator_correlation_matrix_n%d", ++counter), "", final_cov.size(), 1,
                                  final_cov.size(), final_cov.size(), 1, final_cov.size());
    for( int ibin = 1; ibin <= result->GetXaxis()->GetNbins(); ++ibin ) {
      ...
      for( int jbin = ibin; jbin <= jbin_limit; ++jbin ) {
        auto ii = ibin - 1, jj = jbin - 1;
        if( final_cov[ii][ii] == 0.0 || final_cov[jj][jj] == 0.0 )
  ...
  decltype(m_xs) GetTransformedCoordinates() const
  {
    ...
    for( unsigned i = 0; i < DIMENSION; ++i )
      std::for_each(txs.begin(), txs.end(), [this, &i](typename decltype(m_xs)::value_type& p) {
        p.second[i] = this->m_scaling_funcs[i](p.second[i]);
      });
    return std::move(txs);
  }

  std::tuple<decltype(m_ws), decltype(m_ws)> ComputeGridWeightsAndErrors(const decltype(m_bounds)& a,
                                                                         ...
                                                                         const DimArray<double>&   deltas) const
  {
    ...
    for( const auto& x_ : txs ) {
      ...
      // check for bounds
      {
        ...
        for( unsigned i = 0; i < DIMENSION; ++i ) {
          if( x[i] < a[i].first || x[i] > a[i].second )
            out_of_bounds = true;
          if( out_of_bounds )
            break;
        }
      ...
      auto weight = m_ws.at(index), up_error = m_ues.at(index), down_error = m_des.at(index),
      ...
      for( unsigned i = 0; i < DIMENSION; ++i ) {
        ...
        if( x[i] >= extremes[i].first && x[i] <= extremes[i].second ) {
          if( lower_index == upper_index && upper_index == m_number_of_grid_points[i] - 1 )
            --lower_index;
          else if( lower_index == upper_index )
            ++upper_index;
          // this is a special case of a point lying on the lower bound, but numerical precision reduces it just enough
          // to fall in the previous bin's range
          else if( TMath::AreEqualAbs(D, static_cast<double>(upper_index), 1e-10) &&
                   upper_index != m_number_of_grid_points[i] - 1 ) {
            ++lower_index;
            ++upper_index;
          }
        }
        ...
        double lower_portion = (upper_bound - x[i]) / deltas[i], upper_portion = (x[i] - lower_bound) / deltas[i];
      ...
      for( unsigned i = 0; i < pow(2, DIMENSION); ++i ) {
        ...
        double        grid_weight = 1.0, grid_error = 1.0;
  ...
  double GetSelectionNormalization() const
  {
    ...
    if( selection_normalization == 0.0 )
      selection_normalization = m_normalization;
    return selection_normalization;
  }

  BigVector<double> MakeProjection(const decltype(m_ws)& c, const unsigned& dim,
                                   const DimArray<double>& /*deltas*/) const
  {
  ...
  std::tuple<DimArray<SparceCoordList<double>>, DimArray<SparceCoordList<int>>,
             DimArray<SparceCoordList<std::vector<double>>>>
  ComputeBandwidthsRangesAndKernelEvals(const double& selection_normalization, const decltype(m_ws)& c,
                                        const decltype(m_ws)& /*c_error*/, const DimArray<double>&   deltas) const
  {
    ...
    for( unsigned dim = 0; dim < DIMENSION; ++dim ) {
      ...
      if( m_use_adaptive_bandwidth ) {
        // TODO: (maybe) change this to reflect RooNDKeysPDF multidimensional bandwidth (be sure to normalize the kernel
        // evals appropriately)
        int sigma_range =
            TMath::CeilNint(TMath::Power(m_number_of_grid_points[dim], 1. / 3.)); // TODO: make power configurable
        ...
        for( unsigned i = 0; i < projection.size(); ++i ) {
          ...
          if( local_sigma <= 0.01 * this_sigma )
            this_sigma = local_sigma; // TODO: make 0.01 configurable
          // correct units
          // this_sigma *= delta;
          // local_density /= delta;
          // NOTE: cannot ignore zero and negative densities here because this is a projection
          hs[dim][i] = local_density > 0.0 ? knn * constant * TMath::Sqrt(this_sigma / local_density) * deltas[dim]
                                           : m_default_hs[dim] * deltas[dim];
          if( hs[dim][i] > max_h_range )
            hs[dim][i] = max_h_range;
          Ls[dim][i] =
              TMath::Min(TMath::FloorNint(m_taus[dim] * hs[dim][i] / deltas[dim]), m_number_of_grid_points[dim] - 1);
          if( Ls[dim][i] <= 0 )
            Ls[dim][i] = m_number_of_grid_points[dim] - 1;
        ...
        //       the kernel evaluations need to be normalized w.r.t. the bin width
        //       (else the normalization is off)
        auto ks_sum = 0.0, inv_h = 1.0 / hs[dim][0];
  ...
  void ComputeJackknifeErrors(decltype(m_ws)& c, BigVector<double>& fs, BigVector<BigVector<double>>& fs_cov,
                              const DimArray<SparceCoordList<int>>&                 Ls,
                              const DimArray<SparceCoordList<std::vector<double>>>& ks,
                              const decltype(m_bounds)& extremes, const DimArray<double>& deltas) const
  {
    Info("DensityEstimator::ComputeJackknifeErrors",
         "Computing error associated with estimation using the jackknife technique.");
  ...
  TH1* FillSmoothedHistogram(const decltype(m_bounds)& a, const DimArray<double>& deltas, const BigVector<double>& fs,
                             //  const BigVector<double> &fs_error) const
                             const BigVector<BigVector<double>>& fs_cov) const
  {
    if( DIMENSION > 3 )
      return nullptr; // TODO: error message here
    TH1*    result = nullptr;
    ...
    if( DIMENSION == 1 ) {
      result = new TH1D("DensityEstimator_smoothed_histogram", "", m_number_of_grid_points[0], bin_low_edges[0]);
    } else if( DIMENSION == 2 ) {
      result = new TH2D("DensityEstimator_smoothed_histogram", "", m_number_of_grid_points[0], bin_low_edges[0],
                        m_number_of_grid_points[1], bin_low_edges[1]);
    } else if( DIMENSION == 3 ) {
      result = new TH3D("DensityEstimator_smoothed_histogram", "", m_number_of_grid_points[0], bin_low_edges[0],
                        m_number_of_grid_points[1], bin_low_edges[1], m_number_of_grid_points[2], bin_low_edges[2]);
    }
    ...
    DimArray<int> coord;
    auto          total_bins = (result->GetNbinsX() + 2) * (result->GetDimension() >= 1 ? result->GetNbinsY() + 2 : 1) *
                      (result->GetDimension() >= 2 ? result->GetNbinsZ() + 2 : 1);
    for( int ibin = 0; ibin < total_bins; ++ibin ) {
      if( result->IsBinOverflow(ibin) || result->IsBinUnderflow(ibin) )
        continue;
      ...
      if( m_error_type == ErrorEstimationType::Prop || m_error_type == ErrorEstimationType::Jackknife ) {
  ...
  std::tuple<TH1*, BigVector<BigVector<double>>> RebinAndComputeErrors(TH1* result, TH1* binning_histogram,
                                                                       ...
                                                                       const double& sum_weights) const
  {
    ...
    if( new_result->GetDimension() >= 2 )
      new_total_storage *= new_result->GetYaxis()->GetNbins();
    if( new_result->GetDimension() >= 3 )
      new_total_storage *= new_result->GetZaxis()->GetNbins();
    auto                         compute_error = (new_total_storage <= 1000); // TODO: make '1000' configurable
    BigVector<BigVector<double>> final_cov;
    if( compute_error )
      final_cov = BigVector<BigVector<double>>(new_total_storage, BigVector<double>(new_total_storage, 0.0));
    ...
    for( int ixbin = 1; ixbin <= new_result->GetXaxis()->GetNbins(); ++ixbin ) {
      ...
      while( result->GetXaxis()->GetBinUpEdge(old_ixbin) < new_result->GetXaxis()->GetBinLowEdge(1) )
        ++old_ixbin;
      // account for the reverse of above
      if( new_result->GetXaxis()->GetBinUpEdge(ixbin) < result->GetXaxis()->GetBinLowEdge(1) )
        continue;
      ...
      if( new_result->GetDimension() == 1 ) {
        auto new_global_bin = new_result->GetBin(ixbin), old_global_bin = result->GetBin(old_ixbin);

        SparceCoordList<double> a;
        double                  new_weight_sum = 0.0;

        // is old bin within new bin's range?
        while( (old_up_x > new_low_x && old_up_x <= new_up_x) || (old_low_x >= new_low_x && old_low_x < new_up_x) ||
               (old_low_x <= new_low_x && old_up_x >= new_up_x) ) {
          double a_weight = 0.0;

          if( old_low_x >= new_low_x && old_up_x <= new_up_x )
            a_weight = 1.0;
          else if( old_low_x <= new_low_x && old_up_x >= new_up_x )
            a_weight = (new_up_x - new_low_x) / (old_up_x - old_low_x);
          else if( old_low_x < new_low_x && old_up_x > new_low_x && old_up_x <= new_up_x )
            a_weight = (old_up_x - new_low_x) / (old_up_x - old_low_x);
          else if( old_low_x >= new_low_x && old_low_x < new_up_x && old_up_x > new_up_x )
            a_weight = (new_up_x - old_low_x) / (old_up_x - old_low_x);
        ...
        for( int iybin = 1; iybin <= new_result->GetYaxis()->GetNbins(); ++iybin ) {
          // account for old range that extends to lower values than the binning histogram
          while( result->GetYaxis()->GetBinUpEdge(old_iybin) < new_result->GetYaxis()->GetBinLowEdge(1) )
            ++old_iybin;
          // account for the reverse of above
          if( new_result->GetYaxis()->GetBinUpEdge(iybin) < result->GetYaxis()->GetBinLowEdge(1) )
            continue;
          ...
          if( new_result->GetDimension() == 2 ) {
            ...
            while(
                ((old_up_x > new_low_x && old_up_x <= new_up_x) || (old_low_x >= new_low_x && old_low_x < new_up_x) ||
                 (old_low_x <= new_low_x && old_up_x >= new_up_x)) &&
                ((old_up_y > new_low_y && old_up_y <= new_up_y) || (old_low_y >= new_low_y && old_low_y < new_up_y) ||
              ...
              if( old_low_x >= new_low_x && old_up_x <= new_up_x )
                ...
                a_weight = (new_up_x - old_low_x) / (old_up_x - old_low_x);
              if( old_low_y >= new_low_y && old_up_y <= new_up_y )
                a_weight *= 1.0;
              else if( old_low_y <= new_low_y && old_up_y >= new_up_y )
                a_weight *= (new_up_y - new_low_y) / (old_up_y - old_low_y);
              else if( old_low_y < new_low_y && old_up_y > new_low_y && old_up_y <= new_up_y )
                a_weight *= (old_up_y - new_low_y) / (old_up_y - old_low_y);
              else if( old_low_y >= new_low_y && old_low_y < new_up_y && old_up_y > new_up_y )
                a_weight *= (new_up_y - old_low_y) / (old_up_y - old_low_y);
            ...
            if( old_ixbin > result->GetXaxis()->GetNbins() && old_iybin > result->GetYaxis()->GetNbins() )
              break;
          } else {
            auto old_ixbin_origin = old_ixbin;
            auto old_iybin_origin = old_iybin;
            ...
            for( int izbin = 1; izbin <= new_result->GetZaxis()->GetNbins(); ++izbin ) {
              // account for old range that extends to lower values than the binning histogram
              while( result->GetZaxis()->GetBinUpEdge(old_izbin) < new_result->GetZaxis()->GetBinLowEdge(1) )
                ++old_izbin;
              // account for the reverse of above
              if( new_result->GetZaxis()->GetBinUpEdge(izbin) < result->GetZaxis()->GetBinLowEdge(1) )
                continue;
              ...
              while(
                  ...
                  ((old_up_y > new_low_y && old_up_y <= new_up_y) || (old_low_y >= new_low_y && old_low_y < new_up_y) ||
                   (old_low_y <= new_low_y && old_up_y >= new_up_y)) &&
                  ((old_up_z > new_low_z && old_up_z <= new_up_z) || (old_low_z >= new_low_z && old_low_z < new_up_z) ||
                ...
                if( old_low_y >= new_low_y && old_up_y <= new_up_y )
                  ...
                  a_weight *= (new_up_y - old_low_y) / (old_up_y - old_low_y);
                if( old_low_z >= new_low_z && old_up_z <= new_up_z )
                  a_weight *= 1.0;
                else if( old_low_z <= new_low_z && old_up_z >= new_up_z )
                  a_weight *= (new_up_z - new_low_z) / (old_up_z - old_low_z);
                else if( old_low_z < new_low_z && old_up_z > new_low_z && old_up_z <= new_up_z )
                  a_weight *= (old_up_z - new_low_z) / (old_up_z - old_low_z);
                else if( old_low_z >= new_low_z && old_low_z < new_up_z && old_up_z > new_up_z )
                  a_weight *= (new_up_z - old_low_z) / (old_up_z - old_low_z);
              ...
              if( old_ixbin > result->GetXaxis()->GetNbins() && old_iybin > result->GetYaxis()->GetNbins() &&
                  old_izbin > result->GetZaxis()->GetNbins() )
                break;
            }
            ++old_iybin;
          }
    ...
    if( rebinning_normalization != sum_weights ) {
      Info("DensityEstimator::RebinAndComputeErrors",
           "Normalization has been altered during rebinning (%g). Will force proper normalization to %g.",
           rebinning_normalization, sum_weights);
  ...
  TH1* RescaleHistogram(TH1* result, const double& selection_normalization) const
  {
    ...
    for( unsigned dim = 0; dim < DIMENSION; ++dim ) {
      ...
      if( result_bin_extremes[dim].first == result_bin_extremes[dim].second ) {
        ...
        for( int ibin = 1; ibin <= h->GetNbinsX(); ++ibin ) {
          auto low_edge = h->GetXaxis()->GetBinLowEdge(ibin), up_edge = h->GetXaxis()->GetBinUpEdge(ibin);

          if( result_bin_extremes[dim].first == -1 ) {
            if( low_edge <= m_selection_bounds[dim].first && up_edge > m_selection_bounds[dim].first )
              result_bin_extremes[dim].first = ibin;
          } else if( result_bin_extremes[dim].second == -1 ) {
            if( low_edge < m_selection_bounds[dim].second && up_edge >= m_selection_bounds[dim].second )
    ...
    if( DIMENSION == 1 ) {
      new_normalization = selection_normalization * result->Integral("width") /
                          result->Integral(result_bin_extremes[0].first, result_bin_extremes[0].second, "width");
    }
    if( DIMENSION == 2 ) {
      new_normalization =
          selection_normalization * result->Integral("width") /
          static_cast<TH2*>(result)->Integral(result_bin_extremes[0].first, result_bin_extremes[0].second,
                                              result_bin_extremes[1].first, result_bin_extremes[1].second, "width");
    }
    if( DIMENSION == 3 ) {
      new_normalization =
          selection_normalization * result->Integral("width") /
          static_cast<TH3*>(result)->Integral(result_bin_extremes[0].first, result_bin_extremes[0].second,
                                              result_bin_extremes[1].first, result_bin_extremes[1].second,
                                              result_bin_extremes[2].first, result_bin_extremes[2].second, "width");
    }

    // FIXME
    // result->Scale(new_normalization);
    result->Scale(new_normalization / result->Integral("width"));
    Info("DensityEstimator::RescaleHistogram",
         "Integral of smoothed histogram after rescaling (should be close to %.3g): %.3g", new_normalization,
         result->Integral("width"));
 ...
 public:
  DensityEstimator()
  {
    ...
    for( auto& e : m_number_of_grid_points )
      e = 50;
    for( auto& p : m_bounds ) {
      p.first  = 0.0;
      p.second = 0.0;
    }
    for( auto& p : m_selection_bounds ) {
      p.first  = 0.0;
      p.second = 0.0;
    }
    for( auto& e : m_default_hs )
      e = 40;
    for( auto& e : m_taus )
      e = 4.0;
    for( auto& e : m_variance_fractions )
      e = 0.0;
    for( auto& e : m_event_factors )
      e = 1.0;
    for( auto& e : m_scaling_funcs )
      e = TF1("DensityEstimator_default_scale", "x");
    for( auto& e : m_scaling_func_derivatives )
      e = TF1("DensityEstimator_default_scale_dervative", "1");
    for( auto& e : m_inverse_scaling_funcs )
      e = TF1("DensityEstimator_scale_inverse", "x");
    for( auto& e : m_kernels )
      e = TF1("DensityEstimator_kernel", "TMath::Gaus(x, 0, 1, 1)");
    for( auto& e : m_knn_factor_distances )
      e = 0.0;
    for( auto& e : m_knn_factor_settings )
      e = KNNType::Standard;
  }
  ...
  {
    if( h == nullptr )
      return; // TODO: error message here
    if( static_cast<unsigned>(h->GetDimension()) > DIMENSION )
      return; // TODO: error message here
    ...
    for( decltype(temp->GetNbinsX()) ibinx = 1; ibinx <= temp->GetNbinsX(); ++ibinx ) {
      for( decltype(temp->GetNbinsY()) ibiny = 1; ibiny <= temp->GetNbinsY(); ++ibiny ) {
        for( decltype(temp->GetNbinsZ()) ibinz = 1; ibinz <= temp->GetNbinsZ(); ++ibinz ) {
          ...
          if( temp->GetBinContent(global_bin) == 0.0 && temp->GetBinError(global_bin) == 0.0 )
            continue; // only makes sense for sparce data
          ...
          if( dim > 1 )
            bin_volume *= temp->GetYaxis()->GetBinWidth(ibiny);
          if( dim > 2 )
            bin_volume *= temp->GetZaxis()->GetBinWidth(ibinz);
          DimArray<double> x;

          // fill x coordinates
          x[0] = temp->GetXaxis()->GetBinCenter(ibinx);
          if( dim >= 2 )
            x[1] = temp->GetYaxis()->GetBinCenter(ibiny);
          if( dim >= 3 )
            x[2] = temp->GetZaxis()->GetBinCenter(ibinz);
    ...
    for( auto& e : m_ws )
      sum += e.second;
    // Info("DensityEstimator::LoadData", "Input histogram has normalization of %.3g. After rescaling, internal
    // histogram has normalization of %.3g.", m_normalization, sum);

    // clear error if all are zero
    if( std::all_of(m_ues.cbegin(), m_ues.cend(),
                    [](const typename decltype(m_ues)::value_type& p) { return p.second == 0.0; }) &&
        std::all_of(m_des.cbegin(), m_des.cend(),
                    [](const typename decltype(m_ues)::value_type& p) { return p.second == 0.0; }) ) {
      m_ues.clear();
      m_des.clear();
    }
  ...
  void LoadData(const decltype(m_xs)& xs, bool auto_determine_parameters = true)
  {
    const decltype(m_ws) ws;
    for( auto& p : xs )
      ws.emplace(p.first, 1.0);
    this->LoadData(xs, ws, {}, {}, auto_determine_parameters);
  }
  void LoadData(const decltype(m_xs)& xs, const decltype(m_ws)& ws, bool auto_determine_parameters = true)
  {
    this->LoadData(xs, ws, {}, {}, auto_determine_parameters);
  }
  void LoadData(const decltype(m_xs)& xs, const decltype(m_ws)& ws, const decltype(m_ues)& es,
                bool auto_determine_parameters = true)
  {
    this->LoadData(xs, ws, es, es, auto_determine_parameters);
  }
  void LoadData(const decltype(m_xs)& xs, const decltype(m_ws)& ws, const decltype(m_ues)& ues,
                const decltype(m_des)& des, bool auto_determine_parameters = true)
  {
    m_xs            = xs;
    m_ws            = ws;
    m_ues           = ues;
    ...
    m_normalization = 0.0;
    for( const auto& p : ws )
      m_normalization += p.second;
    if( m_normalization > 0.0 )
      for( auto& p : m_ws )
        p.second /= m_normalization;
    else
      return; // TODO: error message here
  ...
  typename std::enable_if<sizeof...(Args_t) >= 2, CoordIndex>::type GetIndex(const Args_t&... args) const
  {
  ...
  template <typename DimArray<int>::size_type DIM> void SetNumberOfGridPoints(const int& m)
  {
    std::get<DIM>(m_number_of_grid_points) = m;
  }
  void SetNumberOfGridPoints(const typename DimArray<int>::size_type& dim, const int& m)
  {
    m_number_of_grid_points[dim] = m;
  }

  template <typename DimArray<int>::size_type DIM> void SetLowerBound(const double& l)
  {
    std::get<DIM>(m_bounds).first = l;
  }
  void SetLowerBound(const typename DimArray<int>::size_type& dim, const double& l) { m_bounds[dim].first = l; }
  template <typename DimArray<int>::size_type DIM> void SetUpperBound(const double& u)
  {
    std::get<DIM>(m_bounds).second = u;
  }
  ...
  template <typename DimArray<int>::size_type DIM> void SetGlobalBandwidth(const double& h)
  {
    m_use_adaptive_bandwidth    = false;
    std::get<DIM>(m_default_hs) = h;
  }
  ...
  template <typename DimArray<int>::size_type DIM> void SetDefaultBandwidth(const double& h)
  {
    m_use_adaptive_bandwidth    = true;
    std::get<DIM>(m_default_hs) = h;
  }
  ...
  template <typename DimArray<int>::size_type DIM> void SetVarianceFraction(const double& f)
  {
    if( f < 0.0 ) {
      Error("DensityEstimator::SetVarianceFraction", "Tried to set a negative variance fraction. Keeping it at %f.",
            std::get<DIM>(m_variance_fractions));
      return;
    }
  ...
  void SetVarianceFraction(const typename DimArray<int>::size_type& dim, const double& f)
  {
    if( f < 0.0 ) {
      Error("DensityEstimator::SetVarianceFraction", "Tried to set a negative variance fraction. Keeping it at %f.",
            m_variance_fractions[dim]);
      return;
    }
    m_variance_fractions[dim] = f;
  }
  template <typename DimArray<int>::size_type DIM> void SetRescaleRange(const double& l, const double& h)
  {
    std::get<DIM>(m_selection_bounds) = l;
    std::get<DIM>(m_selection_bounds) = h;
  }
  void SetRescaleRange(const typename DimArray<int>::size_type& dim, const double& l, const double& h)
  {
    m_selection_bounds[dim] = l;
    m_selection_bounds[dim] = h;
  }
  ...
  template <typename DimArray<int>::size_type DIM> void SetEventFactor(const double& f)
  {
    std::get<DIM>(m_event_factors) = f;
  }
  void SetEventFactor(const typename DimArray<int>::size_type& dim, const double& f) { m_event_factors[dim] = f; }
  template <typename DimArray<int>::size_type DIM> void SetStandardKNN(const double& f)
  {
    std::get<DIM>(m_knn_factor_distances) = f;
    std::get<DIM>(m_knn_factor_settings)  = KNNType::Standard;
  }
  void SetStandardKNN(const typename DimArray<int>::size_type& dim, const double& f)
  {
    m_knn_factor_distances[dim] = f;
    m_knn_factor_settings[dim]  = KNNType::Standard;
  }
  template <typename DimArray<int>::size_type DIM> void SetLargeKNN(const double& f)
  {
    std::get<DIM>(m_knn_factor_distances) = f;
    std::get<DIM>(m_knn_factor_settings)  = KNNType::Large;
  }
  void SetLargeKNN(const typename DimArray<int>::size_type& dim, const double& f)
  {
    m_knn_factor_distances[dim] = f;
    m_knn_factor_settings[dim]  = KNNType::Large;
  }
  template <typename DimArray<int>::size_type DIM> void SetSmallKNN(const double& f)
  {
    std::get<DIM>(m_knn_factor_distances) = f;
    std::get<DIM>(m_knn_factor_settings)  = KNNType::Small;
  }
  void SetSmallKNN(const typename DimArray<int>::size_type& dim, const double& f)
  {
    m_knn_factor_distances[dim] = f;
    m_knn_factor_settings[dim]  = KNNType::Small;
  }

  void UseAdaptiveBandwidth(const bool& f = true) { m_use_adaptive_bandwidth = f; }
  ...
  void SetScalingFunctionsAndDerivative(const typename DimArray<int>::size_type& dim, const TF1& f, const TF1& i,
                                        const TF1& d)
  {
  ...
  template <typename DimArray<int>::size_type DIM> void SetKernelFunction(const TF1& f, const double& tau = 1.0)
  {
    std::get<DIM>(m_kernels) = f;
    std::get<DIM>(m_taus)    = tau;
  }
  ...
  TKernelDensityEstimatorResult CreateSmoothedHistogram(TH1* binning_histogram = nullptr, bool rescale = true) const
  {
    ...
    int total_storage = 1;
    for( const auto& N : m_number_of_grid_points )
      total_storage *= N;
    decltype(m_ws)     c, c_error;
    ...
    if( m_error_type == ErrorEstimationType::Prop && compute_errors ) {
      Info("DensityEstimator::CreateSmoothedHistogram",
           "Computing error associated with estimation using full error propagation.");
    ...
    // TODO: do bootstrap for errors on estimation
      ...
      // Info("DensityEstimator::CreateSmoothedHistogram", "Computing error associated with estimation using the
      // bootstrap technique.");
    } else if( compute_errors )
      Info("DensityEstimator::CreateSmoothedHistogram", "Not computing error associated with estimation.");
    else
      Info("DensityEstimator::CreateSmoothedHistogram",
           "Not computing error associated with estimation due to computational limitations.");
    ...
    // FIXME error matrix far too large
    if( compute_errors )
      _result.CorrelationHistogram = MakeCorrelationHistogram(final_cov);

    Info("DensityEstimator::CreateSmoothedHistogram", "Integral of smoothed histogram (should be close to %.3g): %.3g",
         sum_weights, result->Integral("width"));
...
bool AllZerosOrNegative(const TH1* h)
{
  for( Int_t i = 0; i <= h->GetNbinsX(); ++i ) {
    if( !h->IsBinOverflow(i) && !h->IsBinUnderflow(i) && h->GetBinContent(i) > 0.0 )
      return false;
  }
  return true;
}

std::pair<TH1*, TH2*> smooth_histo(TH1* orighisto, const double& knn, const double& ef, const bool& low_full,
                                   const bool& high_full, const std::string& scale, const double& grid_factor,
                                   const bool& return_corr)
{
  std::pair<TH1*, TH2*> result(nullptr, nullptr);

  if( orighisto == nullptr )
    return result;

  // TH1 *result = nullptr;
  TString orighisto_name(orighisto->GetName()), orighisto_title(orighisto->GetTitle());

  if( AllZerosOrNegative(orighisto) ) {
    result.first = static_cast<TH1*>(orighisto->Clone());
  ...
  if( scale == "log" )
    de.SetScalingFunctionsAndDerivative(0, TF1("scale", "TMath::Log(x)"), TF1("scale_inv", "TMath::Exp(x)"),
...
bool check_for_NaNs_and_Infs(const TH1* h)
{
  for( Int_t ibin = 1; ibin <= h->GetNbinsX(); ++ibin ) {
    if( std::isnan(h->GetBinContent(ibin)) || std::isinf(h->GetBinContent(ibin)) )
      return true;
    if( std::isnan(h->GetBinErrorUp(ibin)) || std::isinf(h->GetBinErrorUp(ibin)) )
      return true;
    if( std::isnan(h->GetBinErrorLow(ibin)) || std::isinf(h->GetBinErrorLow(ibin)) )
      return true;
  }
  return false;
}
