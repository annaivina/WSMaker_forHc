...
The results will be printed as well as stored in a root file in <folder> specified by you (default 'test')
...
bool   doInj               = 1;             // compute expected limit after signal injection
double precision           = 0.005;         // % precision in mu that defines iterative cutoff
bool   verbose             = 0;             // 1 = very spammy
bool   usePredictiveFit    = 1; // experimental, extrapolate best fit nuisance parameters based on previous fit results
bool   extrapolateSigma    = 1; // experimantal, extrapolate sigma based on previous fits
...
// main
void runAsymptoticsCLs(const char* infile, const char* workspaceName, const char* modelConfigName, const char* dataName,
                       const char* asimovDataName, string folder, string mass, double CL, string poiname);
...
void runAsymptoticsCLs(const char* infile, const char* workspaceName = "combWS",
                       ...
                       const char* nominalSnapshot = "nominalGlobs", string folder = "test", string mass = "130",
                       double CL = 0.95, string poiname = "");
...
// RooDataSet* makeAsimovData2(RooDataSet* conditioningData, double mu_true, double mu_prof = -999, string* mu_str =
// NULL, string* mu_prof_str = NULL); RooDataSet* makeAsimovData2(RooNLLVar* conditioningNLL, double mu_true, double
// mu_prof = -999, string* mu_str = NULL, string* mu_prof_str = NULL);

void        unfoldConstraints(RooArgSet& initial, RooArgSet& final, RooArgSet& obs, RooArgSet& nuis, int& counter);
RooDataSet* makeAsimovData(bool doConditional, RooNLLVar* conditioning_nll, double mu_val, string poiname,
                           string* mu_str = NULL, string* mu_prof_str = NULL, double mu_val_profile = -999,
                           bool doFit = true, double mu_injection = -1);

//////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////

void runAsymptoticsCLs(const char* infile, const char* workspaceName, const char* modelConfigName, const char* dataName,
                       const char* asimovDataName, const char* conditionalSnapshot, const char* nominalSnapshot,
                       string folder, string mass, double CL, string poiname)
{
  stringstream smass;
  smass << mass;
  ...
  runAsymptoticsCLs(infile, workspaceName, modelConfigName, dataName, asimovDataName, folder, smass.str(), CL, poiname);
}
                       ...
                       const char* asimovDataName, string folder, string mass, double CL, string poiname)
{
  ...
  // first POI is the POI for which the limit should be calculated
  TIterator*  it = mc->GetParametersOfInterest()->createIterator();
  RooRealVar* n  = (RooRealVar*)it->Next();
  while( n ) {
    TString tmpPOIname = n->GetName();
    if( tmpPOIname == poiname ) {
      cout << "Doing: " << tmpPOIname << endl;
      firstPOI = n;
    }
    n = (RooRealVar*)it->Next();
  }

  dataset = (RooDataSet*)w->data(dataName);
  if( !dataset ) {
  ...
  obs_nll                = createNLL(dataset); //(RooNLLVar*)pdf->createNLL(*data);
  ...
  if( !asimovData_0 ) {
    asimovData_0 = makeAsimovData(conditionalExpected, obs_nll, 0, poiname);
  ...
  double med_limit  = doExp ? getLimit(asimov_0_nll, 0.2) : 1.0;
  ...
  if( doInj ) {
    ...
    //       mu_inj = w->var("ATLAS_norm_muInjection")->getVal();
    //     } else {
    //       mu_inj = w->var("SigXsecOverSM")->getVal(); // This needs to be put in by hand for injected mass = test
    //       mass to be correct
    //     }

    std::cout << "Injected Mu " << mu_inj << std::endl;

    RooDataSet* asimovData_inj =
        makeAsimovData(conditionalExpected, obs_nll, 0, poiname, NULL, NULL, -999, true, mu_inj);
  ...
  if( betterBands && doExp ) // no better time than now to do this
  {
    ...
    for( int N = 2; N >= -2; N-- ) {
      ...
      if( N < 0 && profileNegativeAtZero )
        pr_val = 0;
      RooDataSet* asimovData_N = makeAsimovData(1, asimov_0_nll, NtimesSigma, poiname, &muStr, &muStrPr, pr_val, 0);
  ...
  if( obs_status != 0 || med_status != 0 || asimov0_status != 0 || inj_status != 0 )
    hasFailures = true;
  ...
  fileName << folder << "/" << mass << "_" << poiname << ".root";
...
RooNLLVar* createNLL(RooDataSet* _data)
{
  ...
  if( nuis != 0 )
    nll = (RooNLLVar*)mc->GetPdf()->createNLL(*_data, Constrain(*nuis), NumCPU(nCPU, 3), Optimize(2), Offset(true));
  else
    nll = (RooNLLVar*)mc->GetPdf()->createNLL(*_data, NumCPU(nCPU, 3), Optimize(2), Offset(true));
...
void setMu(double mu)
{
  ...
  if( mu > 0 && firstPOI->getMax() < mu )
    firstPOI->setMax(2 * mu);
  if( mu < 0 && firstPOI->getMin() > mu )
    firstPOI->setMin(2 * mu);
  firstPOI->setVal(mu);
}
...
double getQmu95(double sigma, double mu)
{
  ...
  if( verbose )
    cout << "Returning qmu95 = " << qmu95 << endl;
...
int minimize(RooAbsReal* fcn)
{
  ...
  // call Fits::minimize in macros/minimize.C
  int status = Fits::minimize(fcn);
...
RooDataSet* makeAsimovData(bool doConditional, RooNLLVar* conditioning_nll, double mu_val, string poiname,
                           string* mu_str, string* mu_prof_str, double mu_val_profile, bool doFit, double mu_injection)
{
  ...
  if( doConditional && doFit ) {
    ...
    // Do not print the fit result here, else one gets the unblinded POIs
    int tmp_pl = ROOT::Math::MinimizerOptions::DefaultPrintLevel();
    ROOT::Math::MinimizerOptions::SetDefaultPrintLevel(-1);
    minimize(conditioning_nll);
    ROOT::Math::MinimizerOptions::SetDefaultPrintLevel(tmp_pl);

    // Print out NPs
    TIterator*  itnp  = mc->GetNuisanceParameters()->createIterator();
    RooRealVar* nnp   = (RooRealVar*)itnp->Next();
    TString     atype = getenv("ANALYSISTYPE");
    bool        isVH  = (atype.Contains("VHbb") || atype.Contains("VHcc"));
    std::cout << "Conditional fit result: " << std::endl;

    while( nnp ) {
      TString tmpPOIname = nnp->GetName();
      if( isVH ) {
        // blind signal NPs
        if( !(tmpPOIname.Contains("VH") || tmpPOIname.Contains("Higgs") || tmpPOIname.Contains("Signal") ||
              tmpPOIname.Contains("Sig")) ) {
          std::cout << "--> " << tmpPOIname << ": " << nnp->getVal() << std::endl;
        }
      } else {
        std::cout << "--> " << tmpPOIname << ": " << nnp->getVal() << std::endl;
      }
      nnp = (RooRealVar*)itnp->Next();
    }
  ...
  // for the post-fit Asimov creation, set all POIs to unity
  it = mc->GetParametersOfInterest()->createIterator();
  n  = (RooRealVar*)it->Next();
  while( n ) {
    TString tmpPOIname = n->GetName();
    n->setVal(1.0);
...
void doExpected(bool isExpected)
{
  ...
  bool        blinded = true;
  char const* tmp     = getenv("IS_BLINDED");
  if( tmp == NULL ) {
    cout << "Couldn't getenv IS_BLINDED" << endl;
  } else {
    std::string s(tmp);
    if( s.compare("0") == 0 ) {
      cout << "UnBlind the limit" << endl;
      blinded = false;
    } else
      cout << "Blind the limit" << endl;
  }

  if( isExpected ) {
    doBlind             = blinded;       // in case your analysis is blinded
    ...
    doBlind             = blinded;       // in case your analysis is blinded
    conditionalExpected = 1;             // Profiling mode for Asimov data: 0 = conditional MLEs, 1 = nominal MLEs
    doExp               = 1;             // compute expected limit
    doObs               = 1 && !doBlind; // compute observed limit
  }
}

void doBetterBands(bool isBetterBands) { betterBands = isBetterBands; }

void doInjection(bool injection) { doInj = injection; }
