...
struct Result {
  ...
  Result(bool doObs, float obs_sig, float obs_pValue, float med_sig, float med_pValue, float inj_sig,
         float inj_pValue) :
      obs_sig(doObs ? obs_sig : 0),
      obs_pValue(doObs ? obs_pValue : 0),
      med_sig(med_sig),
      med_pValue(med_pValue),
      inj_sig(inj_sig),
      inj_pValue(inj_pValue)
  {}

  Result() = default;
};

RooDataSet* makeAsimovData(ModelConfig* mc, bool doConditional, RooWorkspace* w, RooNLLVar* conditioning_nll,
                           TString poiStr, double mu_val, string* mu_str, string* mu_prof_str, double mu_val_profile,
                           double mu_injection = -1);
int         minimize(RooNLLVar* nll, RooWorkspace* combWS = NULL);
Result      runSig(const char* inFileName,
                   const char* wsName          = "combined", /// combined",
                   const char* modelConfigName = "ModelConfig", const char* dataName = "obsData",
                   const char* asimov1DataName = "asimovData_1", const char* conditional1Snapshot = "conditionalGlobs_1",
                   const char* nominalSnapshot = "nominalGlobs", TString poiStr = "", string smass = "110",
                   string folder = "test",
                   bool   doExp  = false) // in case your analysis is blinded
{
  double       mass;
  ...
  if( tmp == NULL ) {
    ...
    std::string s(tmp);
    if( s.compare("0") == 0 ) {
      cout << "UnBlind the significance" << endl;
      blinded = false;
    } else
      cout << "Blind the significance" << endl;
  }

  double mu_profile_value = 1;                  // mu value to profile the obs data at before generating the expected
  bool   doConditional    = 1 && !doExp;        // do conditional expected data
  ...
  bool   doObs            = !blinded && !doExp; // compute observed significance only if not blinded and fit to data
  bool   doMedian         = 1; // compute observed significance if fit to data, otherwise compute MC based significance
  ...
  if( poiStr != "" ) {
    ...
    mu = ws->var(poiStr.Data());
    if( !mu ) {
      cout << "Cannot find " << poiStr << " in workspace" << endl;
      return Result();
    }
    cout << "POI" << endl;
    cout << "mu_min: " << mu->getMin() << endl;
    cout << "mu_max: " << mu->getMax() << endl;
    cout << "mu_init: " << mu->getVal() << endl;
  }
  ...
  RooNLLVar*  obs_nll =
      !doExp ? (RooNLLVar*)pdf->createNLL(*data, Constrain(nuis_tmp2), Offset(1), Optimize(2), NumCPU(nCPU, 3)) : NULL;
  ...
  if( !asimovData1 || (string(inFileName).find("ic10") != string::npos && emb) ) {
    ...
    asimovData1  = makeAsimovData(mc, doConditional, ws, obs_nll, poiStr, 1, &mu_str, &mu_prof_str, mu_profile_value);
  ...
  if( doMedian ) {
    // be sure all POIs are free to float
  ...
  if( doInj ) {
    ...
    RooDataSet* injData1 = makeAsimovData(mc, doConditional, ws, obs_nll, poiStr, 0, &mu_str, &mu_prof_str, 1, mu_inj);
  ...
  if( obs_sig != 0 )
    obs_pValue = (1 - TMath::Erf(obs_sig / sqrt(2))) / 2;
  if( med_sig != 0 )
    med_pValue = (1 - TMath::Erf(med_sig / sqrt(2))) / 2;
  if( inj_sig != 0 )
    inj_pValue = (1 - TMath::Erf(inj_sig / sqrt(2))) / 2;
  ...
  Result res(doObs, obs_sig, obs_pValue, med_sig, med_pValue, inj_sig, inj_pValue);
...
int minimize(RooNLLVar* nll, RooWorkspace* combWS)
{
  ...
  // call Fits::minimize of macros/minimize.C
  int status = Fits::minimize(nll);
...
RooDataSet* makeData(RooDataSet* orig, RooSimultaneous* simPdf, const RooArgSet* observables, RooRealVar* firstPOI,
                     double mass, double& mu_min)
{
  ...
  while( (ds = (RooAbsData*)dataItr->Next()) ) {
    ...
    RooDataSet* thisData =
        new RooDataSet(datasetName.str().c_str(), datasetName.str().c_str(), obsAndWeight, WeightVar(*weightVar));
    ...
    for( int ib = 0; ib < nrEntries; ib++ ) {
      ...
      if( s > 0 ) {
        ...
        if( soverb > max_soverb ) {
          max_soverb = soverb;
          cout << "Found new max s/b: " << soverb << " in pdf " << pdf->GetName() << " at m = " << thisObs->getVal()
               << endl;
        }
      }
      ...
      //       }
      if( b == 0 && s != 0 ) {
        cout << "Expecting non-zero signal and zero bg at m=" << firstObs->getVal() << " in pdf " << pdf->GetName()
             << endl;
      }
      if( s + b <= 0 ) {
      ...
      if( (typeName.find("ATLAS_H_4mu") != string::npos || typeName.find("ATLAS_H_4e") != string::npos ||
           typeName.find("ATLAS_H_2mu2e") != string::npos || typeName.find("ATLAS_H_2e2mu") != string::npos) &&
          fabs(firstObs->getVal() - mass) < 10 && weight == 0 ) {
  ...
  RooDataSet* newData = new RooDataSet("newData", "newData", RooArgSet(*observables, *weightVar), Index(*cat),
                                       Import(data_map), WeightVar(*weightVar));
...
void localUnfoldConstraints(RooArgSet& initial, RooArgSet& final, RooArgSet& obs, RooArgSet& nuis, int& counter)
{
  if( counter > 50 ) {
  ...
  while( (pdf = (RooAbsPdf*)itr->Next()) ) {
    ...
    if( className != "RooGaussian" && className != "RooLognormal" && className != "RooGamma" &&
      ...
      counter++;
      localUnfoldConstraints(constraint_set, final, obs, nuis, counter);
    } else {
      final.add(*pdf);
    }
  }
                           ...
                           double mu_injection)
{
  ...
  cout << "setting mu to: " << mu_val << endl;
  mu->setVal(mu_val);
  ...
  int        counter_tmp = 0;
  localUnfoldConstraints(constraint_set_tmp, constraint_set, mc_obs, mc_nuis_tmp, counter_tmp);
  ...
  if( doConditional ) {
    ...
    ROOT::Math::MinimizerOptions::SetDefaultPrintLevel(-1);
    minimize(conditioning_nll, nullptr);
  ...
  cout << "Glob snapshot name = "
       << "conditionalGlobs" + muStrProf.str() << endl;
  cout << "Nuis snapshot name = "
       << "conditionalNuis" + muStrProf.str() << endl;
