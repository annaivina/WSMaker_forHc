...
class WorkspaceConfig(odict):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.regionRegexp = re.compile("^(\\d+)TeV(\\w+)?_(\\w+)_(\\w+)tag(\\w+)jet(\\w+taus)?_(\\d+)(_\\d+)?ptv(_\\w+)?_(\\w+)$")
        self.tagRegexp = re.compile("(\\d+)?([lmtxnba]+)?(p)?")
    ...
    def _line(self, key):
        ...
        try:
            ...
            sys.exit(f"No {key} set ! Aborting...")
    ...
    def create_config(self, name, order=None):
        ...
        if order:
            order = order.split() if isinstance(order, str) else order
        else:
        ...
        try:
            regs = [self["Regions"]] if isinstance(self["Regions"], str) else self["Regions"]
        except KeyError:
    ...
    def check_regions(self):
        ...
        for r in self["Regions"]:
            ...
            m2 = self.tagRegexp.match(m.groups()[3])
            if m2 is None:
                sys.exit("Malformed tag region " + m2.groups()[3])

            m3 = self.jetRegexp.match(m.groups()[4])
            if m is None:
                sys.exit("Malformed jet region " + m.groups()[4])
