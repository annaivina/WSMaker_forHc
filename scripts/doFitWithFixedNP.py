...
def fitPDF(cfg, ws, rfr):
    ...
    print(f'MAX CALLS {maxCalls}\t{ROOT.Math.MinimizerOptions.DefaultMaxFunctionCalls()}')
...
def getComponents(cfg, w):
    ...
    for tt in categories(channelCat):
        ...
        binWidth = pdftmp.getVariables().find(f"binWidth_obs_x_{ttname}_0")
        logging.debug(f"    Bin Width : {binWidth.getVal()}")
