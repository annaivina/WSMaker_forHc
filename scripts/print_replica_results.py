...
def corrplot(param="SigXsec", label="#hat{#mu}", NLep="2" , Error=False, Significance=False):
    ...
    if Significance:
        inpar = param+f"Results{NLep}.txt"
        inerr = param+f"ErrResults{NLep}.txt"
        ...
        infile = param+f"ErrResults{NLep}.txt"
        ...
        infile = param+f"Results{NLep}.txt"
    ...
    outfile = ROOT.TFile(f"{param}/{filename}correlation_CUT_MVA{npoints}.root","recreate")
    tg = ROOT.TGraph(npoints)

    if Significance:
        inpar = param+f"Results{NLep}.txt"
        inerr = param+f"ErrResults{NLep}.txt"
        with open(inpar) as fp, open(inerr) as fe:
    ...
    l1 = new_TLatex()
    l1.DrawLatex(0.6, 0.2, f"Correlation: {corr:.3f}")
    #l2 = new_TLatex()
    #l2.DrawLatex(0.6, 0.15, "Covariance: {0:.3f}".format(cov))

    c.SaveAs(f"{param}/{filename}correlation{npoints}{imgform}")

    hcorr.Write()

    hcut = hcorr.ProjectionX(f"{param} CUT")
    ...
    hcut.Draw("same")
    cutl1 = new_TLatex()
    cutl1.DrawLatex(0.63, 0.90, f"fit Mean: {fcut.GetParameter(1):.3f} +/- {fcut.GetParError(1):.3f}")
    cutl2 = new_TLatex()
    cutl2.DrawLatex(0.63, 0.85, f"fit Sigma: {fcut.GetParameter(2):.3f} +/- {fcut.GetParError(2):.3f}")
    draw_ATLAS_label((0.2,0.8))
    cutchi = new_TLatex()
    cutchi.DrawLatex(0.63,0.80,f"#chi^{{2}}/NDF={fcut.GetChisquare()/fcut.GetNDF():.2f}")
    cutl3 = new_TLatex()
    cutl3.DrawLatex(0.63, 0.75, f"hist Mean: {hcut.GetMean():.3f} +/- {hcut.GetMeanError():.3f}")
    cutl4 = new_TLatex()
    cutl4.DrawLatex(0.63, 0.70, f"hist RMS: {hcut.GetRMS():.3f} +/- {hcut.GetRMSError():.3f}")

    c.SaveAs(f"{param}/{filename}cut{npoints}{imgform}")

    hcut.Write()

    hmva = hcorr.ProjectionY(f"{param} MVA")
    ...
    fmva.Draw("same")
    mval1 = new_TLatex()
    mval1.DrawLatex(0.63, 0.90, f"fit Mean: {fmva.GetParameter(1):.3f} +/- {fmva.GetParError(1):.3f}")
    mval2 = new_TLatex()
    mval2.DrawLatex(0.63, 0.85, f"fit Sigma: {fmva.GetParameter(2):.3f} +/- {fmva.GetParError(2):.3f}")
    draw_ATLAS_label((0.2,0.8))
    mvachi = new_TLatex()
    mvachi.DrawLatex(0.63,0.80,f"#chi^{{2}}/NDF={fmva.GetChisquare()/fmva.GetNDF():.2f}")
    mval3 = new_TLatex()
    mval3.DrawLatex(0.63, 0.75, f"hist Mean: {hmva.GetMean():.3f} +/- {hmva.GetMeanError():.3f}")
    mval4 = new_TLatex()
    mval4.DrawLatex(0.63, 0.70, f"hist RMS: {hmva.GetRMS():.3f} +/- {hmva.GetRMSError():.3f}")

    c.SaveAs(f"{param}/{filename}mva{npoints}{imgform}")
    ...
    fdiff.Draw("same")
    diffl1 = new_TLatex()
    diffl1.DrawLatex(0.63, 0.90, f"fit Mean: {fdiff.GetParameter(1):.3f} +/- {fdiff.GetParError(1):.3f}")
    diffl2 = new_TLatex()
    diffl2.DrawLatex(0.63, 0.85, f"fit Sigma: {fdiff.GetParameter(2):.3f} +/- {fdiff.GetParError(2):.3f}")
    draw_ATLAS_label((0.2,0.8))
    diffchi = new_TLatex()
    diffchi.DrawLatex(0.63,0.80,f"#chi^{{2}}/NDF={fdiff.GetChisquare()/fdiff.GetNDF():.2f}")
    diffl3 = new_TLatex()
    diffl3.DrawLatex(0.63, 0.75, f"hist Mean: {hdiff.GetMean():.3f} +/- {hdiff.GetMeanError():.3f}")
    diffl4 = new_TLatex()
    diffl4.DrawLatex(0.63, 0.70, f"hist RMS: {hdiff.GetRMS():.3f} +/- {hdiff.GetRMSError():.3f}")

    c.SaveAs(f"{param}/{filename}diff{npoints}{imgform}")
...
def corrrep(param="SigXsec", label="#hat{#mu}", NLep="2", Error=False, Significance=False):
    ...
    outfile = ROOT.TFile(f"{param}/{filename}correlation_replicas{npoints}.root","recreate")
    ...
    c.SaveAs(f"{param}/{filename}corrfullrep{npoints}{imgform}")
    ...
    c.SaveAs(f"{param}/{filename}corrnrep{npoints}{imgform}")
...
def meanplot(param="SigXsec", label="#hat{#mu}", NLep = "2", Error=False, Significance=False):
    ...
    outfile = ROOT.TFile(f"{param}/{filename}mean_replicas{npoints}.root","recreate")
    ...
    c.SaveAs(f"{param}/{filename}meanrep{npoints}{imgform}")
