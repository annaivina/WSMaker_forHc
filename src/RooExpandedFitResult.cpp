...
#include "TMatrixDSym.h"   // for TMatrixDSym
...
RooExpandedFitResult::RooExpandedFitResult(RooFitResult* origResult, RooArgList extraPars)
{
  ...
  for( Int_t j = 0; j < extraPars.getSize(); j++ ) {
    ...
    if( !saveFloatInitList.contains(*par) )
      saveFloatInitList.add(*par);
    if( !saveFloatFinalList.contains(*par) )
      saveFloatFinalList.add(*par);
  }
  ...
  for( Int_t ic = 0; ic < saveFloatFinalList.getSize(); ic++ ) {
    ...
    if( origFloatFinalList.contains(*par) )
      globalCC.push_back(origResult->globalCorr(*par));
    else
      globalCC.push_back(0.0);
  }
