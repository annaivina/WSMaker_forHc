...
InputsHandlerPaper::InputsHandlerPaper(const Configuration& conf, const PropertiesSet& pset) : InputsHandler()
{
...
TString InputsHandlerPaper::forgeFileName(const Configuration& conf, const PropertiesSet& pset)
{
...
void InputsHandlerPaper::findBaseDir() { m_baseDir = m_file.get(); }

bool InputsHandlerPaper::checkData()
{
  TH1* data = nullptr;
  m_baseDir->GetObject((std::string("data") + mass).c_str(), data);
  return data != nullptr;
}
