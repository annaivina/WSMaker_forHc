...
OutputConfig::OutputConfig(Configuration& conf)
{
  ...
  if( deleteFiles ) {
    // create a unique temporary location in /tmp
    char normDir_template[] = "/tmp/normfiles.XXXXXX";
    normDir = std::string(mkdtemp(normDir_template)); // this creates the new directory and returns its path
  } else {
    // put them into the output directory instead (access can be slow on some systems)
    normDir = "output/" + fullVersion + "/normfiles/";
  ...
  doGaussianConstraint = conf.getValue("UseGaussianConstraintTypeMCStat", false);
  xmlDir               = TString("output/") + fullVersion + TString("/xml/") + massPoint;
  ...
  workspaceDir = TString("output/") + fullVersion + TString("/workspaces/combined");
  ...
  plotDir = TString("output/") + fullVersion + TString("/plots/");
  ...
  std::system(("cp $ROOTSYS/etc/HistFactorySchema.dtd " + xmlDir).Data());
}
    ...
    m_doGaussianConstraint(false)
{
  ...
  // m_xml.swap(tmpfile);
  m_xml = std::make_unique<std::ofstream>(m_xmlName);
  ...
  *m_xml << "<!DOCTYPE Channel SYSTEM 'HistFactorySchema.dtd'>\n\n"
  ...
  m_doGaussianConstraint = conf->doGaussianConstraint;
}
...
OutputHandler& OutputHandler::operator=(OutputHandler&& other)
{
  ...
  m_doGaussianConstraint = other.m_doGaussianConstraint;
...
void OutputHandler::addData(const TString& name, const TH1* hdata)
{
  ...
  *m_xml << "<Data InputFile=\"" << m_fname << "\" HistoName=\"" << name << "\" HistoPath=\"\" />\n\n";
}
...
void OutputHandler::beginSample(const TString& name, const TH1* hnom)
{
  ...
  *m_xml << "<Sample Name=\"" << name << "\" InputFile=\"" << m_fname << "\" HistoName=\"" << name
...
{
  *m_xml << "<OverallSys Name=\"" << sys.name << "\" High=\"" << sys.var_up << "\" Low=\"" << sys.var_do << "\"/>\n";
}

void OutputHandler::addShapeSyst(const Systematic& sys) const
{
  if( sys.shape_up == nullptr || sys.shape_do == nullptr ) {
    std::cout << std::endl << "ERROR::OutputHandler::addShapeSyst: " << sys.name << std::endl;
  ...
  *m_xml << "<HistoSys Name=\"" << sys.name << "\" HistoNameLow=\"" << sys.shape_do->GetName() << "\" HistoNameHigh=\""
         << sys.shape_up->GetName() << "\" HistoPathLow=\"/" << sys.name << "__1down/\" HistoPathHigh=\"/" << sys.name
         << "__1up/\"/>\n";
...
{
  *m_xml << "<NormFactor Name=\"" << sys.name << "\" Val=\"" << sys.init << "\" Low=\"" << sys.var_do << "\" High=\""
         << sys.var_up <<"\""<<" />\n"; // "\" Const=\"True\" />\n";
}

void OutputHandler::addMCStatSyst(const Systematic& sys) const
{
  if( sys.mc_stat_error_fractional_shared == nullptr ) {
    std::cout << std::endl << "ERROR::OutputHandler::addMCStatSyst: " << sys.name << std::endl;
    std::cout << "  which has no mc_stat_error_fractional_shared histogram" << std::endl;
    std::cout << "  Please FIXME ! I promise it is not hard" << std::endl << std::endl;
    throw;
    // FIXME
  }

  *m_xml << "<ShapeSys Name=\"" << sys.name << "\""
         << " HistoName=\"" << sys.name << "\""
         << " HistoPath=\"/" << sys.name << "/\""
         << " ConstraintType=\"Poisson\""
         << "/>\n";
  writeHisto(sys.name, sys.mc_stat_error_fractional_shared.get(), sys.name);
}

void OutputHandler::addShapeFact(const Systematic& sys) const
{
  *m_xml << "<ShapeFactor Name=\"" << sys.name << "\" />\n";
}
