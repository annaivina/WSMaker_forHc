...
namespace PU {
  ...
  std::map<TString, int> setSysGroups()
  {
    ...
    if( anaType == "boostedVHbbRun2" ) {
      ...
      sysGroups["SysFT"]            = 60;
      sysGroups["SysJET"]           = 70;
      sysGroups["SysFATJET_JER"]    = 72;
      sysGroups["SysFATJET_JMR"]    = 73;
      sysGroups["SysMJJMR"]         = 74;
      sysGroups["SysFATJET_Medium"] = 76;
      sysGroups["Elec"]             = 80;
      sysGroups["SysEG"]            = 80;
      sysGroups["SysEL"]            = 80;
      sysGroups["Muon"]             = 85;
      sysGroups["SysMUON"]          = 85;
      sysGroups["MET"]              = 90;
      sysGroups["L0_ttbar"]         = 100;
      sysGroups["L1_ttbar"]         = 100;
      sysGroups["J0_ttbar"]         = 100;
      sysGroups["J1_ttbar"]         = 100;
      sysGroups["TTbar"]            = 100;
      sysGroups["Systtbar"]         = 100;
      sysGroups["TopP"]             = 100;
      sysGroups["Sysstop"]          = 110;
      sysGroups["stop"]             = 110;
      sysGroups["Stop"]             = 110;
      sysGroups["Wt"]               = 110;
      sysGroups["WMbb"]             = 120;
      sysGroups["WbbMbb"]           = 120;
      sysGroups["WPt"]              = 120;
      sysGroups["SysWb"]            = 120;
      sysGroups["Whf"]              = 120;
      sysGroups["Wcc"]              = 120;
      ...
      sysGroups["Ttbar"]    = 90;
      sysGroups["TTbar"]    = 90;
      sysGroups["Systtbar"] = 90;
      sysGroups["TChan"]    = 100;
      sysGroups["SChan"]    = 100;
      sysGroups["WtChan"]   = 100;
      sysGroups["Sysstop"]  = 100;
      sysGroups["SysMJ"]    = 110;
      sysGroups["LUMI"]     = 210;
      sysGroups["BTag"]     = 220;
      sysGroups["SysFT"]    = 220;
      sysGroups["SysJet"]   = 230;
      sysGroups["SysJET"]   = 230;
      sysGroups["MET"]      = 230;
      sysGroups["Elec"]     = 240;
      sysGroups["Muon"]     = 240;
      sysGroups["SysEG"]    = 240;
      sysGroups["SysMUON"]  = 240;
      sysGroups["SysEL"]    = 240;
    }
    return sysGroups;
  }
  ...
  bool comp_simple(const lab_pos& p1, const lab_pos& p2) { return p1.first < p2.first; }
  ...
  bool comp_sysNames(const lab_pos& p1, const lab_pos& p2)
  {
    ...
    for( std::map<TString, int>::iterator it = sysGroups.begin(); it != sysGroups.end(); ++it ) {
      if( p1.first.Contains(it->first) ) {
        group1 = it->second;
      }
      if( p2.first.Contains(it->first) ) {
        group2 = it->second;
      }
    }
    if( group1 != group2 )
  ...
  void sortTGraphAxis(TGraph* g, TAxis* a, const bool isXaxis, bool (*fcn)(const lab_pos&, const lab_pos&),
                      bool reverse)
  {
    v_lab_pos bins;
    GetBinLabels(a, bins);
  ...
  TGraph* reduceTGraphAxis(const TGraph* g, const TAxis* a, const bool isXaxis, const std::vector<TString>& patterns,
                           bool includes)
  {
  ...
  TGraph* reduceTGraphAxisExclude(const TGraph* g, const TAxis* a, const bool isXaxis,
                                  const std::vector<TString>& excludes)
  {
    return reduceTGraphAxis(g, a, isXaxis, excludes, false);
  }

  TGraph* reduceTGraphAxisInclude(const TGraph* g, const TAxis* a, const bool isXaxis,
                                  const std::vector<TString>& includes)
  {
