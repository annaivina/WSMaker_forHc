...
#include "RooAbsData.h"    // for TMatrixDSym, RooAbsData
#include "RooAbsPdf.h"     // for RooAbsPdf
#include "RooArgSet.h"     // for RooArgSet
#include "RooCurve.h"      // for RooCurve
#include "RooFitResult.h"  // for RooFitResult
#include "RooGlobalFunc.h" // for Normalization, VisualizeError
...
namespace RU {
  ...
  {
    // Plot function or p.d.f. on frame with support for visualization of the uncertainty encoded in the given fit
    // result fr. If params is non-zero, only the subset of the parameters in fr that occur in params is considered for
    // the error evaluation Argument argList can contain any RooCmdArg named argument that can be applied to a regular
    // plotOn() operation
    //
    ...
    // not be accurate in the presence of strong correlations (~>0.9) and at Z>2 due to linear and Gaussian
    // approximations made
    //
    ...
    if( pdfs.size() == 1 ) {
      std::cout << "WARNING::plotOnWithErrorBand: please do not use the complicated way to do simple things !"
                << std::endl;
  ...
  {
    unsigned int ncurves = added.size();
    if( ncurves == 0 ) {
    ...
    for( unsigned int i = 1; i < ncurves; i++ ) {
