...
Sample::Sample(const TString& name, std::vector<TString>&& subsamples, const Type type, const int color,
    ...
    m_type(type),
    m_color(color),
    m_subsamples(std::move(subsamples)),
    m_veto(std::move(veto))
{}

Sample::Sample(const TString& name, const Type type, const int color, VetoFunction&& veto) :
    Sample(name, {}, type, color, std::move(veto))
{}

bool Sample::operator==(const Sample& other) { return m_name == other.m_name; }
...
void Sample::add32Syst(const Systematic32& sys32)
{
  for( const auto& c : categories() ) {
    ...
    if( cat2J == nullptr ) {
      std::cout << "Sample::add32Syst::ERROR: corresponding 2J category not found. Cannot apply 3/2-jet ratio!"
                << std::endl;
      exit(-1);
    }
...
std::set<TString> Sample::addMCStatSyst(SysConfig& conf)
{
  std::set<TString> result;
  for( const auto& c : categories() ) {
    if( !SystematicHandler::match(*(c.first), conf) )
    ...
    // Change name of syst if some decorrelations apply
    TString sysname;
    sysname = "MCStat" + conf.getDecorrelationTag(c.first->properties(), *this);
    result.insert(sysname);
    c.second->addMCStatSyst(sysname);
  }
  return result;
}

bool Sample::isVetoedIn(Category& c) { return m_veto(c); }
