...
void splitter::ReadConfig()
{
  ...
  while( getline(configFile, line) ) {
    ...
    inputFile.file = 0;
    cout << "Found:	" << inputFile.energy << "	" << inputFile.channel << "	" << inputFile.type << "	"
         << inputFile.fileName;
...
void splitter::writeOutFile(InputFile& inputFile, TString tagRegion, TString VpTbin, TString dist)
{
  TString match = tagRegion + "_" + VpTbin + "_" + dist;
  TString outFileName =
      m_outDirTmp + inputFile.energy + "_" + inputFile.channel + "_" + inputFile.type + "_" + match + ".root";
...
void splitter::findRegions(TDirectory* dir, vector<TString>& tagRegions, vector<TString>& vptBins,
                           vector<TString>& distributions, TString distToUse, TString anySample)
{
  ...
  while( (key = (TKey*)nextkey()) ) {
    if( TClass::GetClass(key->GetClassName())->InheritsFrom(TH1::Class()) ) {
      ...
      if( getNameField(name, 0).BeginsWith(anySample) ) {
        ...
        bool    hasUpperVpt = (testField.BeginsWith("90") || testField.BeginsWith("12") || testField.BeginsWith("16") ||
