...
void splitter_Run2::ReadConfig()
{
  ...
  while( getline(configFile, line) ) {
    ...
    if( inputFile.channel.BeginsWith("CoreRegions") ) {
      ...
      cout << "ERROR: Energy tag in input file name not found: " << inputFile.fileName << endl;
      exit(-1);
    }
    inputFile.file = 0;
    cout << "Found:	" << inputFile.energy << "	" << inputFile.channel << "	" << inputFile.fileName;
...
void splitter_Run2::readHistoList(TDirectory* source, TString subdir, const std::set<TString>& distros)
{
  ...
  TPRegexp stricttagpattern("\\w+_(((2\\w*tag(2|3)p?jet\\w*taus(0|1p))|(topemu\\w+))_\\d+(_\\d+)?ptv_\\w+_([a-zA-Z0-9]+))(_\\w+)?");
  while( (key = (TKey*)nextkey()) ) {
    ...
    if( objclass->InheritsFrom(TDirectory::Class()) ) {
      if( name.Contains("Systematics") || name.BeginsWith("Sys") ) {
        if( m_debug )
          cout << "Going into " << key->GetName() << endl;
        cout << ".";
        flush(cout);
    ...
    } else if( objclass->InheritsFrom(TH1::Class()) && !objclass->InheritsFrom(TH2::Class()) &&
               !objclass->InheritsFrom(TH3::Class()) ) {
      
      //For one "Systematics" directory we have histogrms name like: Systematics/ttbar_2btag3jet_150_250ptv_SR_mva_SysJET_Flavor_Composition__1down
      //For one systematics per directory we have histogrms name like: SysJET_Flavor_Composition__1down/ttbar_2btag3jet_150_250ptv_SR_mva
      //For the latter case, adding back the systematics name in order to do the matching.
      if(stripped_subdir != "" && stripped_subdir != "Systematics") name=name+"_"+stripped_subdir;
      TString nameSys(name);
      if( name.Index("_Sys") > 0 )
        nameSys.Remove(0, name.Index("_Sys") + 1);
      TPRegexp   tagpattern("\\w+_((\\w*tag\\w*jet(\\w+taus)?)_(\\d+)(_\\d+)?(?:ptv|ptbb|BDT)(_[a-zA-Z0-9]+)+_(\\w+))" +
                            (stripped_subdir == "" ? "" : "_" + nameSys));
      TObjArray* res = tagpattern.MatchS(name);

      if( res->GetEntries() < 2 ) {
        cout << "WARNING: found an histo with non-matching name " << name << " for dir " << (subdir == "" ? "" : subdir)
             << " !" << endl;
        delete res;
        continue;
      }
      ...
      if( distros.size() ) {
	TString dist = Utils::group(res, 7);
...
void splitter_Run2::SplitFile(InputFile_R2& inputFile)
{
  ...
  TString subdir = "";
  if( m_subDir == "" )
    subdir = inputFile.channel;
  else
    subdir = m_subDir;
  TObject* subDir = inputFile.file->Get(m_subDir);
  if( subDir )
...
void splitter_Run2::writeOutFile(InputFile_R2& inputFile, const TString& regName)
{
  ...
  TString outFileName = m_outDirTmp + inputFile.energy + "_" + inputFile.channel + "_" + match + ".root";
...
void splitter_Run2::copyHistos(TDirectory* source, TDirectory* target, TString match)
{
  ...
  for( auto hName(m_histoList[match].begin()); hName != m_histoList[match].end(); ++hName ) {
    ...
    //just keep the systematic name for one systematic per directory case
	//if( subDir != "" )
...
void splitter_Run2::addEmptyDataHistoIfMissing(const TString& fileName)
{
  ...
  while( (key = (TKey*)nextkey()) ) {
    ...
    if( objclass->InheritsFrom(TH1::Class()) && !objclass->InheritsFrom(TH2::Class()) &&
        !objclass->InheritsFrom(TH3::Class()) ) {
